...
class FixedBitkubAPI:
    ...
    def verify_symbol(self, symbol):
        ...
        if normalized in self.verified_symbols:
            return True, normalized

        ticker = self.get_simple_ticker(normalized)
    ...
    def place_buy_order_safe(self, symbol, amount_thb, buy_price, order_type="limit"):
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"✅ BUY - {trading_symbol}: {amount_thb} @ {buy_price}")
    ...
    def place_sell_order_safe(self, symbol, amount_crypto, sell_price, order_type="limit"):
        """✅ FIXED: Place sell with symbol_map"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"✅ SELL - {trading_symbol}: {amount_crypto} @ {sell_price}")
...
class EnhancedTradingBot(ctk.CTk):
    ...
    def execute_buy(self, symbol, ticker, reason, score=0):
        """✅ FIXED: Execute buy with error tracking"""
        try:
    ...
    def execute_sell(self, symbol, ticker, reason):
        """✅ FIXED: Execute sell with error tracking"""
        try:
            price = ticker['last_price']
