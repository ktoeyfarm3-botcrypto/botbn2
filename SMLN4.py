...
"""
        ...
        if len(self.price_history) >= 25:  # Reduced from 50 to 25
...
"""
    ...
    def train_ai_model(self):
        """Train AI model with enhanced feedback and progress tracking"""

        # Create training progress window
        training_window = tk.Toplevel(self.root)
        training_window.title("🧠 AI Model Training Progress")
        ...
        # Progress frame
        progress_frame = ttk.LabelFrame(training_window, text="การฝึก AI Model")
        ...
        # Status label
        status_label = ttk.Label(progress_frame, text="เตรียมความพร้อม...",
                                 font=('Arial', 11))
        status_label.pack(pady=10)

        # Details text
        details_text = scrolledtext.ScrolledText(progress_frame, height=15,
                                                 bg='#1a1a1a', fg='#ffffff',
                                                 font=('Consolas', 9))
        ...
        close_btn = ttk.Button(btn_frame, text="ปิด", state='disabled',
        ...
        def update_progress(percent, message):
            ...
            details_text.insert(tk.END, f"[{percent:3.0f}%] {message}\n")
        ...
        def train_with_progress():
            ...
            try:
                ...
                details_text.insert(tk.END, "🧠 เริ่มการฝึก AI Model\n")
                details_text.insert(tk.END, "=" * 50 + "\n")
                details_text.insert(tk.END, f"⏰ เวลา: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                details_text.insert(tk.END, f"🎯 Symbol: {self.config['trading_symbol']}\n")
                details_text.insert(tk.END, f"🤖 Model: Random Forest Regressor\n")
                details_text.insert(tk.END, f"📊 Min Data Required: 100 records\n")
                details_text.insert(tk.END, "=" * 50 + "\n\n")

                # Check data availability first - get more data
                all_data = self.db_manager.get_recent_data(self.config['trading_symbol'], limit=2000)
                data_count = len(all_data)

                details_text.insert(tk.END, f"📊 ตรวจสอบข้อมูลในฐานข้อมูล...\n")
                details_text.insert(tk.END, f"📈 Memory Data Points: {len(self.price_history)}\n")
                details_text.insert(tk.END, f"💾 Database Records: {data_count}\n\n")

                if data_count < 100:
                    update_progress(0, f"ข้อมูลไม่เพียงพอ: มี {data_count} รายการ ต้องการ 100 รายการ")
                    details_text.insert(tk.END, f"❌ ข้อมูลไม่เพียงพอสำหรับการฝึก\n")
                    details_text.insert(tk.END, f"📊 ข้อมูลปัจจุบัน: {data_count} รายการ\n")
                    details_text.insert(tk.END, f"📊 ข้อมูลที่ต้องการ: 100 รายการ\n")
                    details_text.insert(tk.END, f"⏳ โปรดรอให้ระบบเก็บข้อมูลเพิ่มเติม\n")
                    details_text.insert(tk.END, f"🔄 ประมาณ {(100 - data_count) * 1:.0f} นาทีจะเก็บข้อมูลครบ\n")

                    # Force save current data if we have memory data but not in DB
                    if len(self.price_history) > data_count and self.api_client:
                        details_text.insert(tk.END, f"\n🔄 กำลังบันทึกข้อมูลจากหน่วยความจำลงฐานข้อมูล...\n")
                        update_progress(10, "บันทึกข้อมูลลงฐานข้อมูล...")

                        # Save current price data to database
                        symbol = self.config['trading_symbol']
                        for i, (price, timestamp) in enumerate(
                                zip(list(self.price_history)[-50:], list(self.time_history)[-50:])):
                            try:
                                # Create basic market data entry
                                basic_indicators = {
                                    'rsi': 50, 'macd': 0, 'macd_signal': 0,
                                    'bb_upper': price, 'bb_middle': price, 'bb_lower': price,
                                    'stoch_k': 50, 'williams_r': -50, 'trend_strength': 0,
                                    'ma_5': price, 'ma_20': price, 'ma_50': price
                                }
                                ...
                                market_data = (
                                    ...
                                    float(price),
                                    1.0,  # default volume
                                    float(price),  # high_24h
                                    float(price),  # low_24h
                                    0.0,  # change_24h
                                    basic_indicators.get('rsi', 50),
                                    basic_indicators.get('macd', 0),
                                    basic_indicators.get('macd_signal', 0),
                                    basic_indicators.get('bb_upper', 0),
                                    basic_indicators.get('bb_middle', 0),
                                    basic_indicators.get('bb_lower', 0),
                                    basic_indicators.get('stoch_k', 50),
                                    basic_indicators.get('williams_r', -50),
                                    basic_indicators.get('trend_strength', 0),
                                    basic_indicators.get('ma_5', 0),
                                    basic_indicators.get('ma_20', 0),
                                    basic_indicators.get('ma_50', 0)
                                )
                                self.db_manager.save_market_data(market_data)

                                if i % 10 == 0:
                                    progress = 10 + (i / 50) * 20
                                    update_progress(progress, f"บันทึกข้อมูล {i + 1}/50...")

                            except Exception as e:
                                details_text.insert(tk.END, f"⚠️ ข้อผิดพลาดในการบันทึก: {str(e)}\n")

                        # Recheck data count
                        new_data_count = len(self.db_manager.get_recent_data(symbol, limit=2000))
                        details_text.insert(tk.END, f"✅ บันทึกเสร็จสิ้น! ข้อมูลใหม่: {new_data_count} รายการ\n")

                        if new_data_count >= 100:
                            details_text.insert(tk.END, f"🎉 ข้อมูลครบแล้ว! เริ่มฝึก Model...\n\n")
                ...
                # Continue with training if we have enough data
                if data_count >= 100:
                    # Start training with progress
                    success, message = self.ml_model.train_model(
                        self.config['trading_symbol'],
                        progress_callback=update_progress
                    )

                    if success:
                        details_text.insert(tk.END, f"\n✅ การฝึก AI Model เสร็จสิ้น!\n")
                        details_text.insert(tk.END, f"📊 รายละเอียด:\n{message}\n")

                        # Show feature importance if available
                        if hasattr(self.ml_model.model, 'feature_importances_'):
                            details_text.insert(tk.END, f"\n📈 Feature Importance (Top 5):\n")
                            feature_names = ['RSI', 'MACD', 'MACD_Signal', 'BB_Upper', 'BB_Middle',
                                             'BB_Lower', 'Stoch_K', 'Williams_R', 'Trend_Strength',
                                             'MA_5', 'MA_20', 'MA_50', 'Price', 'Volume', 'Momentum_5', 'Momentum_10']
                            ...
                            for i, (name, importance) in enumerate(feature_importance[:5]):
                                details_text.insert(tk.END, f"  {i + 1}. {name}: {importance:.4f}\n")

                        self.log_ai_analysis(f"✅ AI Model Training Completed!")
                        self.log_ai_analysis(f"📊 Model Accuracy: {self.ml_model.prediction_accuracy * 100:.1f}%")

                        self.metric_labels["🔥 Model Accuracy"].configure(
                            text=f"{self.ml_model.prediction_accuracy * 100:.1f}%"
                        )
                    else:
                        details_text.insert(tk.END, f"\n❌ การฝึก AI Model ล้มเหลว!\n")
                        details_text.insert(tk.END, f"📋 สาเหตุ: {message}\n")
                ...
                update_progress(0, f"เกิดข้อผิดพลาด: {str(e)}")

            finally:
                close_btn.configure(state='normal')
                details_text.insert(tk.END, f"\n⏰ เสร็จสิ้นเวลา: {datetime.now().strftime('%H:%M:%S')}\n")

        # Start training in separate thread
        threading.Thread(target=train_with_progress, daemon=True).start()
...
Current Price:          {self.price_history[-1] if self.price_history else 0:,.0f} THB
