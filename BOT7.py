...
class SafeTradingBot:
    ...
    def calculate_indicators(self, df):
        ...
        try:
            ...
            df['ema_9'] = calculate_ema(closes, 9)
            df['ema_21'] = calculate_ema(closes, 21)
            ...
            df['macd'] = np.array(ema_12) - np.array(ema_26)
            df['macd_signal'] = calculate_ema(df['macd'].values, 9)

            df['volume_sma'] = calculate_ema(volumes, 20)
...
def place_smart_order(side, strategy="MANUAL"):
    ...
    try:
        ...
        # Calculate SL/TP
        if side == SIDE_BUY:
            stop_loss = fill_price * (1 - settings['stop_loss_percent'] / 100)
            take_profit = fill_price * (1 + settings['take_profit_percent'] / 100)
        else:
            stop_loss = fill_price * (1 + settings['stop_loss_percent'] / 100)
            take_profit = fill_price * (1 - settings['take_profit_percent'] / 100)
        ...
        try:
            ...
            logger.log(f"Stop Loss placed at ${stop_loss:.8f}", 'SUCCESS')
        except Exception as e:
        ...
        try:
            ...
            logger.log(f"Take Profit placed at ${take_profit:.8f}", 'SUCCESS')
        except Exception as e:
...
Auto trading will place real orders. Continue?"""
    ...
    if auto_trading:
        auto_btn.configure(text="🛑 Stop Auto Trading", style="danger")
        ...
        auto_btn.configure(text="🚀 Start Auto Trading", style="success")
...
def open_settings():
    ...
    create_modern_setting_row(risk_frame, "📊 Max Positions", max_pos_var, 1, 10, " positions",
                              "Maximum concurrent open positions")
    create_modern_setting_row(risk_frame, "🚦 Daily Trade Limit", max_daily_var, 1, 50, " trades",
                              "Maximum trades allowed per day")

    # Technical Indicators Tab
    tech_frame = tk.Frame(notebook, bg="#0d1117")
    notebook.add(tech_frame, text="📈 Technical Analysis")

    rsi_os_var = tk.IntVar(value=settings['rsi_oversold'])
    rsi_ob_var = tk.IntVar(value=settings['rsi_overbought'])

    create_modern_setting_row(tech_frame, "📉 RSI Oversold Level", rsi_os_var, 20, 40, "",
                              "RSI level considered oversold (buy signal)")
    create_modern_setting_row(tech_frame, "📈 RSI Overbought Level", rsi_ob_var, 60, 80, "",
                              "RSI level considered overbought (sell signal)")

    # Volume settings
    volume_var = tk.DoubleVar(value=settings['min_volume'] / 1000000)
    create_modern_setting_row(tech_frame, "📊 Minimum Volume", volume_var, 0.1, 50, "M USDT",
                              "Minimum 24h volume required for trading")

    # Notifications Tab
    notif_frame = tk.Frame(notebook, bg="#0d1117")
    notebook.add(notif_frame, text="🔔 Notifications")

    notif_card = ModernCard(notif_frame, title="🔔 Alert Settings")
    notif_card.pack(fill="x", padx=20, pady=20)

    notifications_var = tk.BooleanVar(value=settings['enable_notifications'])

    ttk.Checkbutton(notif_card, text="Enable Trading Notifications",
                    variable=notifications_var).pack(anchor="w", padx=15, pady=10)

    # API Tab
    api_frame = tk.Frame(notebook, bg="#0d1117")
    notebook.add(api_frame, text="🔐 API Settings")

    api_card = ModernCard(api_frame, title="🔐 API Configuration")
    ...
    def reset_daily_counter():
        ...
        logger.log("Daily trade counter reset", 'SUCCESS')

    ModernButton(api_card, text="🔄 Change API Credentials",
                 command=change_api_credentials, style="primary").pack(padx=15, pady=10)
    ModernButton(api_card, text="🔄 Reset Daily Counter",
                 command=reset_daily_counter, style="warning").pack(padx=15, pady=5)

    # Current status
    api_key, api_secret, testnet = config_manager.get_api_credentials()
    status_text = f"Current Environment: {'🧪 Testnet' if testnet else '🔴 Live Trading'}"
    tk.Label(api_card, text=status_text, fg="#74b9ff", bg="#2d3436",
             font=("Segoe UI", 11)).pack(padx=15, pady=10)
    ...
    def save_settings():
        ...
        try:
            ...
            settings['max_positions'] = max_pos_var.get()
            settings['rsi_oversold'] = rsi_os_var.get()
            settings['rsi_overbought'] = rsi_ob_var.get()
            settings['min_volume'] = volume_var.get() * 1000000
            settings['enable_notifications'] = notifications_var.get()
            bot.max_daily_trades = max_daily_var.get()

            # Validation
            if settings['risk_percent'] > 5:
                if not messagebox.askyesno("High Risk Warning",
                                           f"Risk per trade is {settings['risk_percent']}%.\n"
                                           "This is considered high risk. Continue?"):
                    return
...
def show_market_scanner():
    """Modern market scanner interface"""
    scanner_window = tk.Toplevel(root)
    scanner_window.title("🔍 Market Scanner")
    scanner_window.geometry("1400x900")
    scanner_window.configure(bg="#0d1117")

    # Modern header
    header = tk.Frame(scanner_window, bg="#1f2937", height=100)
    header.pack(fill="x")
    header.pack_propagate(False)
    ...
    tk.Label(header_content, text="🔍 Advanced Market Scanner",
             font=("Segoe UI", 18, "bold"), fg="#74b9ff", bg="#1f2937").pack(side="left")

    # Scan controls
    control_card = ModernCard(scanner_window, title="🎛️ Scan Configuration")
    control_card.pack(fill="x", padx=20, pady=15)

    control_content = tk.Frame(control_card, bg="#2d3436")
    control_content.pack(fill="x", padx=15, pady=15)

    min_volume_var = tk.StringVar(value="1.0")
    tk.Label(control_content, text="Minimum Volume (M USDT):", fg="white", bg="#2d3436",
             font=("Segoe UI", 10)).pack(side="left")
    volume_entry = tk.Entry(control_content, textvariable=min_volume_var, width=10,
                            bg="#636e72", fg="white", insertbackground="white", relief="flat")
    volume_entry.pack(side="left", padx=10)

    # Results display
    results_card = ModernCard(scanner_window, title="📊 Scan Results")
    results_card.pack(fill="both", expand=True, padx=20, pady=15)

    # Modern treeview
    style.configure("Modern.Treeview", background="#2d3436", foreground="white", fieldbackground="#2d3436")
    style.configure("Modern.Treeview.Heading", background="#1f2937", foreground="#74b9ff",
                    font=("Segoe UI", 10, "bold"))

    tree_frame = tk.Frame(results_card, bg="#2d3436")
    tree_frame.pack(fill="both", expand=True, padx=15, pady=15)

    columns = ('Symbol', 'Price', 'Change%', 'Volume', 'RSI', 'Signal', 'Confidence', 'Action')
    scanner_tree = ttk.Treeview(tree_frame, columns=columns, show='headings',
                                height=25, style="Modern.Treeview")

    for col in columns:
        scanner_tree.heading(col, text=col)
        scanner_tree.column(col, width=150)

    scanner_tree.pack(side="left", fill="both", expand=True)

    scrollbar = ttk.Scrollbar(tree_frame, orient="vertical", command=scanner_tree.yview)
    scrollbar.pack(side="right", fill="y")
    scanner_tree.configure(yscrollcommand=scrollbar.set)

    def scan_market():
        """Enhanced market scanning with detailed progress"""
        if not client:
            show_modern_notification("Connect API first", "warning")
            return

        logger.log("Starting market scan...", 'AUTO')

        try:
            min_vol = float(min_volume_var.get()) * 1000000

            symbols_to_scan = ["BTCUSDT", "ETHUSDT", "ADAUSDT", "DOTUSDT", "LINKUSDT",
            ...
            for symbol in symbols_to_scan:
                try:
                    scanned_count += 1
                    logger.log(f"Scanning {symbol} ({scanned_count}/{len(symbols_to_scan)})", 'AUTO')
                    ...
                    if len(df) > 14:
                        ...
                        if signal == 'BUY' and confidence > 0.7:
                            ...
                            action_text = "Consider Long"
                        elif signal == 'SELL' and confidence > 0.7:
                            signal_text = "🔴 STRONG SELL"
                            action_text = "Consider Short"
                        elif signal:
                            signal_text = f"🟡 WEAK {signal}"
                            action_text = "Monitor"
                        else:
                            signal_text = "⚪ HOLD"
                        ...
                        scan_results.append((
                            symbol, price, change_percent, volume / 1000000,
                            latest_rsi, signal_text, confidence, action_text
                        ))

                except Exception as e:
                    logger.log(f"Error analyzing {symbol}: {e}", 'ERROR')

            # Sort by confidence
            scan_results.sort(key=lambda x: x[6], reverse=True)

            # Display results with color coding
            for result in scan_results:
                symbol, price, change, vol, rsi, signal_text, conf, action = result

                item = scanner_tree.insert('', 'end', values=(
                    symbol, f"${price:,.4f}", f"{change:+.2f}%", f"{vol:.1f}M",
                    f"{rsi:.1f}", signal_text, f"{conf:.2f}", action
                ))

                # Color coding
                if "STRONG BUY" in signal_text:
                    scanner_tree.set(item, 'Signal', "🟢 STRONG BUY")
                elif "STRONG SELL" in signal_text:
                    scanner_tree.set(item, 'Signal', "🔴 STRONG SELL")

            logger.log(f"Market scan completed - {len(scan_results)} symbols analyzed", 'SUCCESS')

        except Exception as e:
            logger.log(f"Market scan error: {e}", 'ERROR')

    # Scan button
    ModernButton(control_content, text="🚀 Start Market Scan",
                 command=lambda: threading.Thread(target=scan_market, daemon=True).start(),
                 style="success").pack(side="right", padx=20)
...
style.configure("success.Modern.TButton", background="#00b894")
style.configure("danger.Modern.TButton", background="#e17055")
...
strategy_card = ModernCard(left_panel, title="🧠 Strategy Engine")
strategy_card.pack(fill="x", pady=10)

strategy_content = tk.Frame(strategy_card, bg="#2d3436")
strategy_content.pack(fill="x", padx=15, pady=15)

strategy_text = """🔹 Multi-Signal Analysis Engine
🔹 RSI + MACD + EMA + Bollinger Bands
🔹 Intelligent Risk Management
🔹 Auto SL/TP with Position Sizing
🔹 Volume Confirmation System
🔹 Enhanced Safety Protocols"""

tk.Label(strategy_content, text=strategy_text, fg="#74b9ff", bg="#2d3436",
         font=("Segoe UI", 9), justify="left").pack(fill="x")
...
def change_symbol():
    ...
    # Search section
    search_card = ModernCard(symbol_window, title="🔍 Search Symbols")
    search_card.pack(fill="x", padx=20, pady=15)

    search_content = tk.Frame(search_card, bg="#2d3436")
    search_content.pack(fill="x", padx=15, pady=15)

    search_entry = tk.Entry(search_content, font=("Segoe UI", 11), bg="#636e72", fg="white",
                            insertbackground="white", relief="flat", bd=5)
...
def show_trading_history():
    """Enhanced trading history interface"""
    ...
    tk.Label(header, text="📜 Complete Trading History",
             font=("Segoe UI", 16, "bold"), fg="#74b9ff", bg="#1f2937").pack(pady=25)

    # Filter controls
    filter_card = ModernCard(history_window, title="🔍 Filters")
    filter_card.pack(fill="x", padx=20, pady=15)

    filter_content = tk.Frame(filter_card, bg="#2d3436")
    filter_content.pack(fill="x", padx=15, pady=15)

    tk.Label(filter_content, text="Symbol:", fg="white", bg="#2d3436",
             font=("Segoe UI", 10)).pack(side="left")
    symbol_filter = ttk.Combobox(filter_content, values=["ALL", "BTCUSDT", "ETHUSDT", "ADAUSDT"])
    symbol_filter.set("ALL")
    symbol_filter.pack(side="left", padx=10)

    tk.Label(filter_content, text="Period:", fg="white", bg="#2d3436",
             font=("Segoe UI", 10)).pack(side="left", padx=(20, 5))
    days_filter = ttk.Combobox(filter_content, values=["1", "7", "30", "90", "ALL"])
    days_filter.set("30")
    days_filter.pack(side="left", padx=5)

    # Results display
    results_card = ModernCard(history_window, title="📊 Trade Records")
    results_card.pack(fill="both", expand=True, padx=20, pady=15)
    ...
    def update_history():
        ...
        if symbol_filter.get() != "ALL":
            query += ' AND symbol = ?'
            params.append(symbol_filter.get())

        if days_filter.get() != "ALL":
            days_ago = datetime.now() - timedelta(days=int(days_filter.get()))
            query += ' AND timestamp >= ?'
        ...
        for trade in trades:
            tree.insert('', 'end', values=trade[1:])

        logger.log(f"Displayed {len(trades)} trade records", 'INFO')

    update_history()

    ModernButton(filter_content, text="🔄 Update", command=update_history,
                 style="primary").pack(side="right", padx=20)
...
def show_portfolio_analysis():
    ...
    tk.Label(header, text="📈 Portfolio Performance Analysis",
             font=("Segoe UI", 16, "bold"), fg="#74b9ff", bg="#1f2937").pack(pady=25)

    # Portfolio chart
    portfolio_card = ModernCard(portfolio_window, title="📊 Performance Charts")
    portfolio_card.pack(fill="both", expand=True, padx=20, pady=15)

    fig_portfolio = Figure(figsize=(14, 10), dpi=100, facecolor="#0d1117")

    if pnl_history and len(pnl_history) > 1:
        ax_pnl = fig_portfolio.add_subplot(211)
        ax_cumulative = fig_portfolio.add_subplot(212)

        x_pnl = range(len(pnl_history))

        # PnL chart with gradient
        ax_pnl.plot(x_pnl, pnl_history, color="#00ff88", linewidth=3, label="Unrealized PnL")
        ax_pnl.fill_between(x_pnl, pnl_history, alpha=0.3, color="#00ff88")

        # Cumulative PnL
        cumulative_pnl = np.cumsum(pnl_history)
        ax_cumulative.plot(x_pnl, cumulative_pnl, color="#74b9ff", linewidth=3, label="Cumulative PnL")
        ax_cumulative.fill_between(x_pnl, cumulative_pnl, alpha=0.2, color="#74b9ff")
...
Peak PnL: ${max_pnl:+,.2f}
Trough PnL: ${min_pnl:+,.2f}
Average PnL: ${avg_pnl:+,.2f}
Volatility: ${np.std(pnl_history):,.2f}'''

        ax_pnl.text(0.02, 0.98, stats_text,
                    transform=ax_pnl.transAxes, color='white',
                    fontsize=11, verticalalignment='top', fontweight='bold',
                    bbox=dict(boxstyle="round,pad=0.5", facecolor="#1f2937", alpha=0.9))

        # Modern chart styling
        for ax in [ax_pnl, ax_cumulative]:
            ax.set_facecolor("#0d1117")
            ax.tick_params(colors="white", labelsize=10)
            ax.grid(True, alpha=0.2, color="white")
            ax.legend(fontsize=11)

        ax_pnl.set_title("💰 Unrealized PnL Tracking", color="white", fontsize=14, fontweight='bold')
        ax_cumulative.set_title("📈 Cumulative Performance", color="white", fontsize=14, fontweight='bold')

    else:
        ax = fig_portfolio.add_subplot(111)
        ax.text(0.5, 0.5,
                '📊 Portfolio Analysis\n\nNo PnL data available yet\n🔐 Connect API and start trading to see performance',
                transform=ax.transAxes, ha='center', va='center',
                color='white', fontsize=14, fontweight='bold')
        ax.set_facecolor("#0d1117")

    fig_portfolio.patch.set_facecolor("#0d1117")

    canvas_portfolio = FigureCanvasTkAgg(fig_portfolio, master=portfolio_card)
    canvas_portfolio.get_tk_widget().pack(fill="both", expand=True, padx=15, pady=15)
