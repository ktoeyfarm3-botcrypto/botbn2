...
class AdvancedMultiCoinStrategy:
    ...
    def __init__(self, api_client, log_callback=None):
        ...
        # ✅ NEW: Order Type & Locked Coins
        self.order_type = "limit"  # "limit" หรือ "market"
        self.locked_coins = set()  # เหรียญที่ล็อคไว้ไม่ให้เทรด
        ...
        if os.path.exists(self.data_save_file):
            try:
                with open(self.data_save_file, 'rb') as f:
                    ...
                    self.trade_history = deque(saved_data.get('trade_history', []), maxlen=1000)
    ...
    def filter_tradeable_coins(self):
        ...
        try:
            ...
            for symbol, data in self.coin_data.items():
                try:
                    # ✅ NEW: ตรวจสอบ locked coins
                    if symbol in self.locked_coins:
                        continue
    ...
    def calculate_rsi(self, prices, period=14):
        """✅ COMPLETE: Calculate RSI with full validation"""
        # ตรวจสอบข้อมูลพื้นฐาน
        if len(prices) < period + 1:
            return 50  # ข้อมูลไม่พอ

        try:
            # ตรวจสอบว่าราคาเปลี่ยนแปลงหรือไม่
            recent_prices = prices[-period - 1:]
            if len(set(recent_prices)) <= 2:
                # ราคาเกือบไม่เปลี่ยน (มีแค่ 1-2 ค่า)
                return 50

            deltas = np.diff(recent_prices)

            # ตรวจสอบว่ามีการเปลี่ยนแปลงจริงๆ
            if np.allclose(deltas, 0, atol=1e-10):
                return 50  # ราคาไม่เปลี่ยน
            ...
            if avg_loss == 0 and avg_gain == 0:
                return 50  # ไม่มีการเปลี่ยนแปลง

            if avg_loss == 0:
                return 95  # มี gain อย่างเดียว → Overbought

            if avg_gain == 0:
                return 5  # มี loss อย่างเดียว → Oversold (แต่ไม่ใช่ 0)
            ...
            rsi = max(1, min(99, rsi))  # จำกัด 1-99 (ไม่ให้ 0 หรือ 100)
    ...
    def find_best_coin_to_buy(self, tradeable_coins):
        ...
        self.ai_log(f"🎯 Analyzing {len(tradeable_coins)} coins...", "STRATEGY")
        ...
        for symbol in tradeable_coins:
            ...
            # ✅ CRITICAL: ข้ามเหรียญที่ RSI ผิดปกติ
            if rsi <= 0 or rsi > 100 or not np.isfinite(rsi):
                self.ai_log(f"⚠️ Invalid RSI: {rsi:.1f} for {symbol}", "STRATEGY")
                continue

            # ✅ NEW: กรอง RSI < 15 ออกเลย
            if rsi < 15:
                self.ai_log(f"⚠️ Extremely low RSI: {rsi:.1f} for {symbol} - SKIP", "STRATEGY")
                continue

            # ✅ IMPROVED: ระวัง RSI ที่ต่ำ (15-25)
            if rsi < 25:
                self.ai_log(f"⚠️ Very low RSI: {rsi:.1f} for {symbol}", "STRATEGY")
                score += 20  # ลดเหลือ 20
            elif rsi > 90:
                continue
            elif rsi < 30:
            ...
            if score > best_score:
                ...
                if score >= self.buy_threshold:
                    ...
                    self.ai_log(f"🎯 Candidate: {coin_name} Score={score}", "STRATEGY")
                    self.ai_log(f"📊 RSI: {rsi:.1f}", "STRATEGY")
                    self.ai_log(f"📈 Volatility: {volatility:.3f}", "STRATEGY")
                    if ml_boost != 0:
                        self.ai_log(f"🧠 ML Boost: {ml_boost:+d}", "STRATEGY")
        ...
        if best_score >= self.buy_threshold:
            coin_name = best_coin.replace("_thb", "").upper()
            self.ai_log(f"✅ READY TO BUY: {coin_name}", "STRATEGY")
            self.ai_log(f"🎯 Final Score: {best_score}", "STRATEGY")
            self.ai_log(f"🎚️ Threshold: {self.buy_threshold}", "STRATEGY")
            ...
            self.ai_log(f"⏸️ No strong signal", "STRATEGY")
            self.ai_log(f"📊 Best score: {best_score}/{self.buy_threshold}", "STRATEGY")
    ...
    def should_sell_profitable(self, current_price, volume):
        ...
        # ✅ Trailing Stop (ถ้าเคยกำไร)
        if profit_pct > 0:  # เคยกำไร
            drawdown = (highest_price - current_price) / highest_price
            if drawdown > 0.01:  # ลดลงจาก peak มากกว่า 1%
                return True, f"Trailing stop (peak: {highest_price:.4f})"
...
class SeparateLogWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent):
        ...
        # ========== BOTTOM: SYSTEM LOGS (4 columns) ==========
        system_log_frame = ctk.CTkFrame(main_container)
        system_log_frame.pack(fill="both", expand=True)

        ctk.CTkLabel(
            system_log_frame,
            text="💻 SYSTEM LOGS (4 Columns)",
            font=("Arial", 13, "bold"),
            text_color="#00aaff"
        ).pack(pady=(5, 5))

        log_grid = ctk.CTkFrame(system_log_frame)
        log_grid.pack(fill="both", expand=True, padx=5, pady=(0, 10))

        # Configure 4 columns
        for i in range(4):
            log_grid.grid_columnconfigure(i, weight=1)
        log_grid.grid_rowconfigure(0, weight=0)
        log_grid.grid_rowconfigure(1, weight=1)

        # System Log
        ctk.CTkLabel(log_grid, text="SYSTEM", font=("Arial", 10, "bold"),
                     text_color="#00aaff").grid(row=0, column=0, pady=(0, 2), padx=(0, 2))
        self.system_log = ctk.CTkTextbox(log_grid, font=("Courier", 9, "bold"),
                                         fg_color="#0a0a0a", text_color="#ffffff",
                                         border_width=2, border_color="#00aaff")
        self.system_log.grid(row=1, column=0, sticky="nsew", padx=(0, 2))

        # Scan Log
        ctk.CTkLabel(log_grid, text="SCAN DETAIL", font=("Arial", 10, "bold"),
                     text_color="#ffaa00").grid(row=0, column=1, pady=(0, 2), padx=2)
        self.scan_log = ctk.CTkTextbox(log_grid, font=("Courier", 9, "bold"),
                                       fg_color="#1a1000", text_color="#ffaa00",
                                       border_width=2, border_color="#ffaa00")
        self.scan_log.grid(row=1, column=1, sticky="nsew", padx=2)

        # Strategy Log (NEW)
        ctk.CTkLabel(log_grid, text="STRATEGY", font=("Arial", 10, "bold"),
                     text_color="#ff00ff").grid(row=0, column=2, pady=(0, 2), padx=2)
        self.strategy_log = ctk.CTkTextbox(log_grid, font=("Courier", 9, "bold"),
                                           fg_color="#1a001a", text_color="#ff00ff",
                                           border_width=2, border_color="#ff00ff")
        self.strategy_log.grid(row=1, column=2, sticky="nsew", padx=2)

        # Performance Log (NEW)
        ctk.CTkLabel(log_grid, text="PERFORMANCE", font=("Arial", 10, "bold"),
                     text_color="#00ff00").grid(row=0, column=3, pady=(0, 2), padx=(2, 0))
        self.performance_log = ctk.CTkTextbox(log_grid, font=("Courier", 9, "bold"),
                                              fg_color="#001a00", text_color="#00ff00",
                                              border_width=2, border_color="#00ff00")
        self.performance_log.grid(row=1, column=3, sticky="nsew", padx=(2, 0))

        # Initialize
        self.system_log.insert("end", "=== SYSTEM LOG ===\n\n")
        self.scan_log.insert("end", "=== SCAN DETAIL LOG ===\n\n")
        self.strategy_log.insert("end", "=== STRATEGY LOG ===\n\n")
        self.performance_log.insert("end", "=== PERFORMANCE LOG ===\n\n")
    ...
    def add_strategy_log(self, message):
        """Add message to strategy log"""
        timestamp = datetime.now().strftime("%H:%M:%S")
        self.strategy_log.insert("end", f"[{timestamp}] {message}\n")
        self.strategy_log.see("end")

        lines = self.strategy_log.get("1.0", "end").split("\n")
        if len(lines) > 200:
            self.strategy_log.delete("1.0", "100.0")

    def add_performance_log(self, message):
        """Add message to performance log"""
        timestamp = datetime.now().strftime("%H:%M:%S")
        self.performance_log.insert("end", f"[{timestamp}] {message}\n")
        self.performance_log.see("end")

        lines = self.performance_log.get("1.0", "end").split("\n")
        if len(lines) > 200:
            self.performance_log.delete("1.0", "100.0")
    ...
    def clear_all_logs(self):
        ...
        # ✅ NEW
        self.strategy_log.delete("1.0", "end")
        self.strategy_log.insert("end", "=== STRATEGY LOG CLEARED ===\n\n")

        self.performance_log.delete("1.0", "end")
        self.performance_log.insert("end", "=== PERFORMANCE LOG CLEARED ===\n\n")
...
class EnhancedTradingBot(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.best_trade = 0
        self.worst_trade = 0
        self.best_trade_symbol = ""
        self.worst_trade_symbol = ""
    ...
    def setup_ui(self):
        ...
        # ✅ IMPROVED: System Logs (4 columns) - STRATEGY + PERFORMANCE
        ...
        # Container for 4 log columns
        log_container = ctk.CTkFrame(left_panel)
        log_container.pack(fill="both", expand=True, padx=10, pady=(5, 10))

        # Configure grid for 4 equal columns
        for i in range(4):
            log_container.grid_columnconfigure(i, weight=1)
        log_container.grid_rowconfigure(0, weight=0)  # Header row
        log_container.grid_rowconfigure(1, weight=1)  # Log row

        # Column 1: SYSTEM (เดิม)
        sys_header = ctk.CTkLabel(
            log_container,
            text="SYSTEM",
            font=("Arial", 9, "bold"),
            text_color="#00aaff"
        )
        sys_header.grid(row=0, column=0, pady=(0, 2), sticky="ew", padx=(0, 2))
        ...
        self.log_display.grid(row=1, column=0, sticky="nsew", padx=(0, 2))

        # Column 2: SCAN DETAIL (เดิม)
        ...
        scan_header.grid(row=0, column=1, pady=(0, 2), sticky="ew", padx=2)
        ...
        self.scan_detail_display.grid(row=1, column=1, sticky="nsew", padx=2)

        # Column 3: STRATEGY LOG (ใหม่)
        ...
        strategy_header.grid(row=0, column=2, pady=(0, 2), sticky="ew", padx=2)

        self.strategy_log_display = ctk.CTkTextbox(
            log_container,
            font=("Courier", 8, "bold"),
            fg_color="#1a001a",
            text_color="#ff00ff",
        ...
        self.strategy_log_display.grid(row=1, column=2, sticky="nsew", padx=2)

        # Column 4: PERFORMANCE LOG (ใหม่)
        ...
        perf_header.grid(row=0, column=3, pady=(0, 2), sticky="ew", padx=(2, 0))

        self.performance_log_display = ctk.CTkTextbox(
            log_container,
            font=("Courier", 8, "bold"),
            fg_color="#001a00",
            text_color="#00ff00",
        ...
        self.performance_log_display.grid(row=1, column=3, sticky="nsew", padx=(2, 0))

        # Initialize all logs
        self.log_display.insert("end", "=== SYSTEM LOG ===\n")
        self.log_display.insert("end", "Bot initialized\n\n")

        self.scan_detail_display.insert("end", "=== SCAN DETAIL LOG ===\n")
        self.scan_detail_display.insert("end", "Waiting for scan...\n\n")

        self.strategy_log_display.insert("end", "=== STRATEGY LOG ===\n")
        self.strategy_log_display.insert("end", "Waiting for analysis...\n\n")

        self.performance_log_display.insert("end", "=== PERFORMANCE LOG ===\n")
        self.performance_log_display.insert("end", "No trades yet\n\n")
        ...
        # ========== ORDER TYPE & LOCKED COINS FRAME ==========
        order_frame = ctk.CTkFrame(self.scrollable_right, fg_color="#1a001a", border_width=2, border_color="#ff00ff")
        order_frame.pack(fill="x", padx=10, pady=10)

        ctk.CTkLabel(
            order_frame,
            text="⚙️ ORDER SETTINGS",
            font=("Arial", 12, "bold"),
            text_color="#ff00ff"
        ).pack(pady=(5, 5))

        # Order Type Selection
        order_type_frame = ctk.CTkFrame(order_frame, fg_color="transparent")
        ...
        self.order_type_var = ctk.StringVar(value="limit")

        order_radio_frame = ctk.CTkFrame(order_type_frame, fg_color="transparent")
        order_radio_frame.pack(fill="x", padx=5, pady=3)

        ctk.CTkRadioButton(
            order_radio_frame,
            text="Limit Order (ปลอดภัย)",
        ...
        self.order_type_label = ctk.CTkLabel(
            order_type_frame,
            text="Current: Limit Order",
            font=("Arial", 8),
            text_color="#00ff00"
        )
        self.order_type_label.pack(anchor="w", padx=5, pady=2)

        # Locked Coins Section
        locked_frame = ctk.CTkFrame(order_frame, fg_color="#2a0000", border_width=1, border_color="#ff0000")
        locked_frame.pack(fill="x", padx=10, pady=5)

        ctk.CTkLabel(
            locked_frame,
            text="🔒 LOCKED COINS (ไม่เทรด)",
            font=("Arial", 10, "bold"),
            text_color="#ff0000"
        ).pack(pady=(5, 2))

        # Input for adding locked coins
        lock_input_frame = ctk.CTkFrame(locked_frame, fg_color="transparent")
        lock_input_frame.pack(fill="x", padx=5, pady=3)

        self.lock_coin_entry = ctk.CTkEntry(
            lock_input_frame,
            width=200,
            height=28,
            placeholder_text="Enter symbol (e.g., btc_thb)",
            font=("Arial", 9)
        )
        self.lock_coin_entry.pack(side="left", padx=2)

        ctk.CTkButton(
            lock_input_frame,
            text="Lock",
            command=self.add_locked_coin,
            height=28,
            width=60,
            fg_color="#cc0000",
            font=("Arial", 9, "bold")
        ...
        ctk.CTkButton(
            lock_input_frame,
            text="Unlock",
            command=self.remove_locked_coin,
            height=28,
            width=60,
            fg_color="#00aa00",
            font=("Arial", 9, "bold")
        ...
        self.locked_coins_display = ctk.CTkTextbox(
            locked_frame,
            font=("Courier", 8),
            fg_color="#1a0000",
            text_color="#ff6666",
        ...
        self.locked_coins_display.pack(fill="x", padx=5, pady=3)
        self.locked_coins_display.insert("end", "No locked coins\n")

        # Quick lock buttons for popular coins
        quick_lock_frame = ctk.CTkFrame(locked_frame, fg_color="transparent")
        quick_lock_frame.pack(fill="x", padx=5, pady=3)
        ...
        popular_coins = ["btc_thb", "eth_thb", "bnb_thb", "sol_thb", "xrp_thb"]
        for coin in popular_coins:
            ctk.CTkButton(
                quick_lock_frame,
                text=coin.replace("_thb", "").upper(),
                command=lambda c=coin: self.quick_lock_coin(c),
                height=22,
                width=40,
                fg_color="#aa0000",
                font=("Arial", 7)
        ...
        # ✅ BUY THRESHOLD
        threshold_container = ctk.CTkFrame(self.manual_settings_frame, fg_color="transparent")
        threshold_container.pack(fill="x", padx=5, pady=2)

        threshold_header = ctk.CTkFrame(threshold_container, fg_color="transparent")
        ...
        self.buy_threshold_label = ctk.CTkLabel(
            threshold_header,
            text="45",
            font=("Arial", 10, "bold"),
            text_color="#00ff00",
        ...
        self.buy_threshold_label.pack(side="right")

        self.buy_threshold_slider = ctk.CTkSlider(
            threshold_container,
            from_=30,
            to=80,
            number_of_steps=50,
        ...
        self.buy_threshold_slider.pack(fill="x", pady=(2, 5))

        # ✅ TAKE PROFIT
        tp_container = ctk.CTkFrame(self.manual_settings_frame, fg_color="transparent")
        tp_container.pack(fill="x", padx=5, pady=2)

        tp_header = ctk.CTkFrame(tp_container, fg_color="transparent")
        tp_header.pack(fill="x")
        ...
        self.tp_slider = ctk.CTkSlider(
            tp_container,
            from_=1,
            to=10,
            number_of_steps=90,
            command=self.update_tp_label
        )
        self.tp_slider.set(2.5)
        self.tp_slider.pack(fill="x", pady=(2, 5))

        # ✅ STOP LOSS
        sl_container = ctk.CTkFrame(self.manual_settings_frame, fg_color="transparent")
        sl_container.pack(fill="x", padx=5, pady=2)

        sl_header = ctk.CTkFrame(sl_container, fg_color="transparent")
        ...
        self.sl_slider = ctk.CTkSlider(
            ...
            to=5,
            number_of_steps=40,
            command=self.update_sl_label
        )
        self.sl_slider.set(2.0)
        self.sl_slider.pack(fill="x", pady=(2, 5))

        # ✅ Current Values Display
        current_values_frame = ctk.CTkFrame(self.manual_settings_frame, fg_color="#001a1a", border_width=1,
                                            border_color="#00ffff")
        current_values_frame.pack(fill="x", padx=5, pady=(5, 2))
        ...
        ctk.CTkLabel(
            ...
            font=("Arial", 9, "bold"),
            text_color="#00aaff"
        ).pack(anchor="w", padx=5, pady=(3, 2))

        self.saved_values_label = ctk.CTkLabel(
            current_values_frame,
            text="Threshold: 45 | TP: 2.5% | SL: 2.0%",
            font=("Arial", 8),
            text_color="#00ff00"
        )
        self.saved_values_label.pack(anchor="w", padx=5, pady=(0, 3))
    ...
    def ai_log(self, message, log_type="AI"):
        """✅ FIXED: Thread-safe unified logging"""

        # ✅ ตรวจสอบว่าเรียกจาก background thread หรือไม่
        if threading.current_thread() != threading.main_thread():
            # ถ้าเรียกจาก background thread → ส่งไปทำที่ main thread
            try:
                self.after(0, lambda m=message, lt=log_type: self._ai_log_internal(m, lt))
            except:
                # ถ้า after ใช้ไม่ได้ (widget ถูกทำลาย) ให้ print แทน
                print(f"[{log_type}] {message}")
            return

        # ถ้าเรียกจาก main thread → ทำงานตรงๆ
        self._ai_log_internal(message, log_type)

    def _ai_log_internal(self, message, log_type="AI"):
        """Internal logging method - ทำงานใน main thread เท่านั้น"""
        timestamp = datetime.now().strftime("%H:%M:%S")
        ...
        if log_type == "STRATEGY":
            try:
                if hasattr(self, 'strategy_log_display'):
                    self.strategy_log_display.insert("end", f"[{timestamp}] {message}\n")
                    self.strategy_log_display.see("end")
                    lines = self.strategy_log_display.get("1.0", "end").split("\n")
                    if len(lines) > 100:
                        self.strategy_log_display.delete("1.0", "50.0")
            except:
                pass

            # Log window
            if self.log_window and self.log_window.winfo_exists():
                self.log_window.add_strategy_log(message)
        ...
        if log_type == "PERFORMANCE":
            try:
                if hasattr(self, 'performance_log_display'):
                    self.performance_log_display.insert("end", f"[{timestamp}] {message}\n")
                    self.performance_log_display.see("end")
                    lines = self.performance_log_display.get("1.0", "end").split("\n")
                    if len(lines) > 100:
                        self.performance_log_display.delete("1.0", "50.0")
            except:
                pass

            # Log window
            if self.log_window and self.log_window.winfo_exists():
                self.log_window.add_performance_log(message)
    ...
    def change_order_type(self):
        """Change order type between limit and market"""
        order_type = self.order_type_var.get()

        if self.strategy:
            self.strategy.order_type = order_type

            if order_type == "limit":
                self.order_type_label.configure(
                    text="Current: Limit Order (ปลอดภัย)",
                    text_color="#00ff00"
                )
                self.ai_log("Order Type: LIMIT (ราคาที่กำหนด)", "SYSTEM")
            else:
                self.order_type_label.configure(
                    text="Current: Market Order (เร็ว, อาจมี Slippage)",
                    text_color="#ff8800"
                )
                self.ai_log("Order Type: MARKET (ราคาตลาด)", "SYSTEM")
        else:
            self.ai_log("Connect API first", "SYSTEM")

    def add_locked_coin(self):
        """Add coin to locked list"""
        if not self.strategy:
            self.ai_log("Connect API first", "SYSTEM")
            return

        symbol = self.lock_coin_entry.get().strip().lower()
        if not symbol:
            return

        # Normalize symbol
        if not symbol.endswith("_thb") and not symbol.startswith("thb_"):
            symbol = f"{symbol}_thb"

        # Add to locked set
        self.strategy.locked_coins.add(symbol)

        # Update display
        self.update_locked_coins_display()

        # Clear entry
        self.lock_coin_entry.delete(0, "end")

        self.ai_log(f"🔒 Locked: {symbol}", "SYSTEM")

    def remove_locked_coin(self):
        """Remove coin from locked list"""
        if not self.strategy:
            self.ai_log("Connect API first", "SYSTEM")
            return
        ...
        if symbol in self.strategy.locked_coins:
            self.strategy.locked_coins.remove(symbol)
            ...
            self.ai_log(f"🔓 Unlocked: {symbol}", "SYSTEM")
        else:
            self.ai_log(f"Coin not locked: {symbol}", "SYSTEM")

    def quick_lock_coin(self, symbol):
        """Quick lock popular coin"""
        if not self.strategy:
            self.ai_log("Connect API first", "SYSTEM")
            return
    ...
    def update_locked_coins_display(self):
        """Update locked coins display"""
        if not self.strategy:
            return

        self.locked_coins_display.delete("1.0", "end")

        if not self.strategy.locked_coins:
            self.locked_coins_display.insert("end", "No locked coins\n")
        else:
            self.locked_coins_display.insert("end", "🔒 Locked Coins:\n")
            for coin in sorted(self.strategy.locked_coins):
                coin_name = coin.replace("_thb", "").upper()
                self.locked_coins_display.insert("end", f"  • {coin_name}\n")
    ...
    def apply_strategy_settings(self):
        ...
        if self.strategy_mode.get() == "manual":
            ...
            if self.strategy:
                ...
                old_threshold = self.strategy.buy_threshold
                old_tp = self.strategy.take_profit * 100
                old_sl = abs(self.strategy.stop_loss) * 100
                ...
                # ✅ แสดง feedback ว่าค่าอะไรเปลี่ยน
                changes = []
                if old_threshold != buy_threshold:
                    changes.append(f"Threshold: {old_threshold}→{buy_threshold}")
                if abs(old_tp - (take_profit * 100)) > 0.01:
                    changes.append(f"TP: {old_tp:.1f}%→{take_profit * 100:.1f}%")
                if abs(old_sl - (stop_loss * 100)) > 0.01:
                    changes.append(f"SL: {old_sl:.1f}%→{stop_loss * 100:.1f}%")

                if changes:
                    self.ai_log(f"✅ Manual Settings Applied:", "SYSTEM")
                    for change in changes:
                        self.ai_log(f"   {change}", "SYSTEM")
                else:
                    self.ai_log(f"ℹ️ No changes (same values)", "SYSTEM")

                # ✅ อัปเดต saved values display
                self.saved_values_label.configure(
                    text=f"Threshold: {buy_threshold} | TP: {take_profit * 100:.1f}% | SL: {stop_loss * 100:.1f}%",
                    text_color="#00ff00"
                )
                ...
                messagebox.showinfo(
                    "Settings Applied",
                    f"Manual settings saved!\n\n"
                    f"Buy Threshold: {buy_threshold}\n"
                    f"Take Profit: {take_profit * 100:.2f}%\n"
            ...
            if self.strategy:
                ...
                # ✅ แสดงค่าที่ AI กำลังใช้
                current_threshold = self.strategy.buy_threshold
                self.ai_log(f"Current AI Threshold: {current_threshold}", "SYSTEM")
                messagebox.showinfo(
                    "Auto Mode",
                    f"Full Auto AI Mode is active.\n\n"
                    f"AI is using adaptive threshold.\n"
                    f"Current: {current_threshold}"
                )
            else:
                self.ai_log("Connect API first", "SYSTEM")

        # ✅ อัปเดต data points settings
        try:
            ...
            if self.strategy:
                ...
                self.ai_log(f"Data Points: Min={min_points:,}, Max={max_points:,}", "SYSTEM")
        except:
            self.ai_log("Invalid data points value", "SYSTEM")

    def update_threshold_label(self, value):
        """Update buy threshold label"""
        try:
            val = int(float(value))
            self.buy_threshold_label.configure(text=str(val))
        except:
            pass

    def update_tp_label(self, value):
        """Update take profit label"""
        try:
            val = float(value)
            self.tp_label.configure(text=f"{val:.2f}%")
        except:
            pass

    def update_sl_label(self, value):
        """Update stop loss label"""
        try:
            val = float(value)
            self.sl_label.configure(text=f"{val:.2f}%")
        except:
            pass
    ...
    def execute_buy(self, symbol, ticker, reason, score):
        """✅ COMPLETE: Execute buy with limit/market order support"""
        try:
            ...
            self.strategy.position = {
                ...
                'amount': amount,
                'buy_fee': buy_fee,
                'amount_thb_spent': amount_thb,  # ✅ NEW: เก็บเงินที่จ่ายจริง
                'pending': True,
                'entry_score': score,
            ...
            if self.is_paper_trading:
                ...
                # ✅ NEW: เช็คว่าใช้ order type อะไร
                order_type = self.strategy.order_type  # "limit" หรือ "market"

                if order_type == "market":
                    # Market Order - ซื้อที่ราคาตลาดทันที
                    self.ai_log(f"REAL MODE - Sending MARKET order...", "AI")
                    buy_price = round(price * 1.005, 8)  # เผื่อ slippage เล็กน้อย
                    result = self.api_client.place_buy_order_safe(symbol, amount_thb, buy_price, 'market')
                else:
                    # Limit Order - ซื้อที่ราคาที่กำหนด (เดิม)
                    self.ai_log(f"REAL MODE - Sending LIMIT order...", "AI")
                ...
                if result.get('error') != 0:
                    ...
                    if balance_result and balance_result.get('error') == 0:
                        ...
                        if actual_amount > 0:
                            ...
                            # ✅ FIXED: คำนวณราคาจริงที่ได้
                            actual_price = amount_thb / actual_amount if actual_amount > 0 else price
                            self.strategy.position['entry_price'] = actual_price
                            self.strategy.position['amount_thb_spent'] = amount_thb  # ✅ อัปเดตอีกครั้ง

                            self.ai_log(f"Actual Amount: {actual_amount:.8f}", "AI")
                            self.ai_log(f"Actual Price: {actual_price:.4f} THB", "AI")
                            self.ai_log(f"Total Spent: {amount_thb:.2f} THB", "AI")  # ✅ NEW
    ...
    def execute_sell(self, symbol, ticker, reason):
        """✅ COMPLETE: Execute sell with limit/market order support"""
        try:
            is_valid, verified_symbol = self.api_client.verify_symbol(symbol)
            if not is_valid:
                self.ai_log(f"Invalid symbol: {symbol}", "ERROR")
            ...
            # ✅ FIXED: ประกาศ coin_name ก่อนใช้
            coin_name = symbol.split('_')[0].upper()
            ...
            # ✅ CRITICAL: ใช้เงินที่จ่ายจริงจาก position
            amount_thb_spent = position.get('amount_thb_spent', 0)
            if amount_thb_spent == 0:
                # Fallback: คำนวณจาก entry_price * amount
                amount_thb_spent = entry_price * amount

            # ✅ CRITICAL: คำนวณเงินที่ได้จากการขาย (ก่อนหัก fee)
            gross_sell_value = price * amount

            # ✅ คำนวณ sell fee
            sell_fee = self.api_client.calculate_trading_fees(amount, price, "sell")

            # ✅ เงินที่ได้จริง (หลังหัก fee)
            net_sell_value = gross_sell_value - sell_fee

            # ✅ FINAL P/L = เงินที่ได้ - เงินที่จ่าย
            net_pnl = net_sell_value - amount_thb_spent
            profit_pct = (net_pnl / amount_thb_spent) * 100 if amount_thb_spent > 0 else 0

            self.ai_log(f"SELL: {reason[:20]}", "AI")
            self.ai_log(f"Entry: {entry_price:.4f} THB", "AI")
            self.ai_log(f"Exit: {price:.4f} THB", "AI")
            self.ai_log(f"Amount: {amount:.8f} {coin_name}", "AI")
            self.ai_log(f"─────────────", "AI")
            self.ai_log(f"Spent: {amount_thb_spent:.2f} THB", "AI")
            self.ai_log(f"Gross Sell: {gross_sell_value:.2f} THB", "AI")
            self.ai_log(f"Sell Fee: {sell_fee:.2f} THB", "AI")
            self.ai_log(f"Net Receive: {net_sell_value:.2f} THB", "AI")
            self.ai_log(f"─────────────", "AI")
            ...
            if self.is_paper_trading:
                ...
                if order_type == "market":
                    # Market Order - ขายที่ราคาตลาดทันที
                    self.ai_log(f"Sending MARKET sell order...", "AI")
                    sell_price = round(price * 0.995, 8)  # เผื่อ slippage เล็กน้อย
                    result = self.api_client.place_sell_order_safe(symbol, amount, sell_price, 'market')
                else:
                    # Limit Order - ขายที่ราคาที่กำหนด (เดิม)
                    self.ai_log(f"Sending LIMIT sell order...", "AI")
            ...
            if net_pnl > 0:
                ...
                self.win_trades.append(net_pnl)
            else:
                self.visual.set_trading_state("LOSS")
                # ✅ NEW: Track loss
                self.total_loss += abs(net_pnl)
                self.loss_trades.append(net_pnl)

                # ✅ NEW: Track all trades
            ...
            # ✅ NEW: Log performance statistics
    ...
    def log_performance_stats(self):
        ...
        try:
            ...
            avg_win = self.total_profit / len(self.win_trades) if self.win_trades else 0
            avg_loss = self.total_loss / len(self.loss_trades) if self.loss_trades else 0
            ...
            if len(self.all_trades) >= 2:
                ...
                if len(recent_trades) >= 2:
                    ...
                    for i in range(1, len(recent_trades)):
                        diff = (recent_trades[i]['timestamp'] - recent_trades[i - 1]['timestamp']).total_seconds() / 60
                        time_diffs.append(diff)
                    avg_hold_time = sum(time_diffs) / len(time_diffs) if time_diffs else 0
                else:
                    avg_hold_time = 0
            else:
                avg_hold_time = 0
            ...
            # ✅ Log to PERFORMANCE
            self.ai_log(f"═══ STATISTICS ═══", "PERFORMANCE")
            self.ai_log(f"📊 Win Rate: {win_rate:.1f}% ({self.win_count}W/{self.trade_count - self.win_count}L)",
                        "PERFORMANCE")

            if avg_win > 0:
                self.ai_log(f"💰 Avg Win: +{avg_win:.0f} THB", "PERFORMANCE")
            if avg_loss > 0:
                self.ai_log(f"💸 Avg Loss: -{avg_loss:.0f} THB", "PERFORMANCE")

            if win_loss_ratio > 0:
                self.ai_log(f"🎯 W/L Ratio: {win_loss_ratio:.2f}", "PERFORMANCE")

            if avg_hold_time > 0:
                self.ai_log(f"⏰ Avg Hold: {avg_hold_time:.0f} min", "PERFORMANCE")

            self.ai_log(f"📈 Total P/L: {self.total_pnl:+,.0f} THB", "PERFORMANCE")

            if self.best_trade > 0:
                self.ai_log(f"🔥 Best: +{self.best_trade:.0f} ({self.best_trade_symbol})", "PERFORMANCE")
            if self.worst_trade < 0:
                self.ai_log(f"💔 Worst: {self.worst_trade:.0f} ({self.worst_trade_symbol})", "PERFORMANCE")

            # Check for streaks
            if len(self.all_trades) >= 3:
                recent = self.all_trades[-3:]
                if all(t['pnl'] > 0 for t in recent):
                    self.ai_log(f"🔥 Streak: 3 wins!", "PERFORMANCE")
                elif all(t['pnl'] < 0 for t in recent):
                    self.ai_log(f"⚠️ Streak: 3 losses", "PERFORMANCE")

            self.ai_log(f"─────────────────", "PERFORMANCE")
    ...
    def update_chart(self):
        ...
        try:
            ...
            if len(self.price_history) > 1:
                ...
                self.ax.legend(loc='upper left', facecolor='#0a0a0a', edgecolor='#00ffff', labelcolor='#00ffff',
                               fontsize=8)
