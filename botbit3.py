# === เพิ่มฟังก์ชันที่เหลือ ===
    
    def create_ai_trading_tab(self, notebook):
        """แท็บ AI Trading ที่ง่ายขึ้น"""
        ...
        basic_settings = [
            ...
            ("กลยุทธ์:", "ai_strategy_var", ["momentum", "mean_reversion"]),
        ...
        for i, (label, var_name, default) in enumerate(numeric_settings, len(basic_settings)):
            ttk.Label(config_grid, text=label, style='Header.TLabel').grid(row=i, column=0, 
                                                                         sticky='w', padx=5, pady=5)
    ...
    def create_manual_trading_tab(self, notebook):
        """แท็บ Manual Trading ที่ง่ายขึ้น"""
        frame = ttk.Frame(notebook)
        ...
        self.trading_result = scrolledtext.ScrolledText(result_frame, height=8, 
                                                      bg='#1e1e1e', fg='#ffffff', 
                                                      font=('Consolas', 10),
                                                      insertbackground='white')
    ...
    def create_enhanced_trading_form(self, parent, side):
        ...
        if side == 'buy':
            ttk.Button(help_frame, text="💰 ใช้ยอดเงินทั้งหมด", 
                      command=lambda: self.use_max_balance(side)).pack(side='left', padx=2)
            ttk.Button(help_frame, text="📊 ราคาตลาด", 
                      command=lambda: self.use_market_price(side)).pack(side='left', padx=2)
        else:
            ttk.Button(help_frame, text="🪙 ขายทั้งหมด", 
                      command=lambda: self.use_max_crypto(side)).pack(side='left', padx=2)
            ttk.Button(help_frame, text="📊 ราคาตลาด", 
                      command=lambda: self.use_market_price(side)).pack(side='left', padx=2)
        ...
        # เพิ่มข้อมูลการคำนวณ
        calc_frame = ttk.Frame(form_frame)
        calc_frame.grid(row=len(fields)+2, column=0, columnspan=2, sticky='ew')
        
        setattr(self, f'{side}_calc_label', ttk.Label(calc_frame, text=""))
        getattr(self, f'{side}_calc_label').pack()
    
    def create_portfolio_tab(self, notebook):
        """แท็บ Portfolio แบบง่าย"""
        frame = ttk.Frame(notebook)
        notebook.add(frame, text="💼 Portfolio")
        
        # ภาพรวม Portfolio
        overview_frame = ttk.LabelFrame(frame, text="💰 ภาพรวม Portfolio")
        overview_frame.pack(fill='x', padx=20, pady=20)
        
        # Balance cards
        balance_container = ttk.Frame(overview_frame)
        balance_container.pack(fill='x', padx=10, pady=10)
        
        self.balance_cards = {}
        currencies = ["THB", "BTC", "ETH", "มูลค่ารวม"]
        
        for i, currency in enumerate(currencies):
            card_frame = ttk.Frame(balance_container, relief='solid', borderwidth=1)
            card_frame.grid(row=0, column=i, padx=5, pady=5, sticky='ew')
            balance_container.grid_columnconfigure(i, weight=1)
            
            ttk.Label(card_frame, text=f"💰 {currency}", style='Header.TLabel').pack(pady=2)
            self.balance_cards[currency] = ttk.Label(card_frame, text="0.00000000", 
        ...
        ttk.Button(control_frame, text="🔄 รีเฟรช Portfolio", 
                  command=self.refresh_portfolio).pack(side='left', padx=5)
        ttk.Button(control_frame, text="📊 ข้อมูลครบถ้วน", 
                  command=self.get_full_balances).pack(side='left', padx=5)
        
        # รายละเอียด Portfolio
        detail_frame = ttk.LabelFrame(frame, text="📊 รายละเอียด Portfolio")
        detail_frame.pack(fill='both', expand=True, padx=20, pady=20)
        
        self.portfolio_detail = scrolledtext.ScrolledText(detail_frame, bg='#1e1e1e', fg='#ffffff', 
                                                        font=('Consolas', 10))
        self.portfolio_detail.pack(fill='both', expand=True, padx=10, pady=10)
    
    def create_market_data_tab(self, notebook):
        """แท็บ Market Data แบบง่าย"""
        frame = ttk.Frame(notebook)
        notebook.add(frame, text="📈 Market Data")
        
        # ควบคุม Market Data
        control_frame = ttk.LabelFrame(frame, text="🎯 ควบคุม Market Data")
        control_frame.pack(fill='x', padx=20, pady=20)
        
        control_container = ttk.Frame(control_frame)
        control_container.pack(fill='x', padx=10, pady=10)
        
        ttk.Label(control_container, text="Symbol:").pack(side='left')
        ...
        symbol_combo = ttk.Combobox(control_container, textvariable=self.market_data_symbol_var, 
                                   values=["btc_thb", "eth_thb", "ada_thb"], state='readonly')
        symbol_combo.pack(side='left', padx=10)
        
        ttk.Button(control_container, text="📊 Ticker", 
                  command=self.get_ticker_data).pack(side='left', padx=5)
        ttk.Button(control_container, text="📋 Order Book", 
                  command=self.get_orderbook_data).pack(side='left', padx=5)
        ttk.Button(control_container, text="💹 Recent Trades", 
                  command=self.get_trades_data).pack(side='left', padx=5)
        
        # แสดงข้อมูล Market
        data_frame = ttk.LabelFrame(frame, text="📊 ข้อมูล Market")
        data_frame.pack(fill='both', expand=True, padx=20, pady=20)
        
        self.market_data_text = scrolledtext.ScrolledText(data_frame, bg='#1e1e1e', fg='#00d4aa', 
                                                        font=('Consolas', 10))
        self.market_data_text.pack(fill='both', expand=True, padx=10, pady=10)
    
    def create_settings_tab(self, notebook):
        """แท็บการตั้งค่าง่าย ๆ"""
        frame = ttk.Frame(notebook)
        ...
        settings = [
            ...
            ("📊 จำนวนข้อมูลราคาที่เก็บ",import tkinter as tk
