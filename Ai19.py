...
class EnhancedTradingBot(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        self.start_button = ctk.CTkButton(
            control_frame,
            text="START",
            command=lambda: self.toggle_trading(),  # ✅ แก้ไขตรงนี้
            height=40,
            font=("Arial", 13, "bold"),
            fg_color="#00aa00"
        )
        ...
        # ✅ FIXED: เพิ่ม lambda ทุกปุ่ม
        ctk.CTkButton(
            btn_frame,
            text="TEST API",
            command=lambda: self.test_api_only(),  # ✅ แก้ไขตรงนี้
            height=28,
            width=140,
            fg_color="#0066cc"
        ...
        ctk.CTkButton(
            btn_frame,
            text="TEST SCAN",
            command=lambda: self.test_scan_small(),  # ✅ แก้ไขตรงนี้
            height=28,
            width=140,
            fg_color="#9900cc"
        ).pack(side="left", padx=2)
        ...
        ctk.CTkButton(
            btn_frame,
            text="BACKTEST",
            command=lambda: self.run_backtest(),  # ✅ แก้ไขตรงนี้
            height=28,
            width=140,
            fg_color="#ff8800"
        ).pack(side="left", padx=2)
    ...
    def toggle_trading(self):
        """Toggle trading with enhanced safety for Real Mode"""
        if not self.api_client or not self.strategy:
            self.ai_log("Connect API first")
            return
        ...
        if self.is_trading:
            ...
            if not self.is_paper_trading:
                ...
                if balance.get('error') == 0:
                    ...
                    self.ai_log("❌ Cannot check balance")
                    self.is_trading = False
                    return

                # Final confirmation
                final_warning = messagebox.askyesno(
                    "🔴 FINAL WARNING - REAL TRADING",
                    f"START REAL MONEY TRADING?\n\n"
                    f"💰 Available: {available:,.2f} THB\n"
                    f"💸 Per Trade: {self.trade_amount_thb} THB\n"
                    f"📊 Max Trades: Unlimited\n"
                    f"🪙 Coins: {len(self.api_client.all_bitkub_symbols)}\n\n"
                    f"⚠️ RISKS:\n"
                    f"• Real money will be used\n"
                    f"• Orders sent to Bitkub\n"
                    f"• Possible losses\n"
                    f"• No guarantees\n\n"
                    f"PROCEED WITH REAL TRADING?"
                )
            ...
            if self.is_paper_trading:
                ...
                self.ai_log("🔴 REAL TRADING STARTED")
                self.ai_log("⚠️ Monitor closely!")
    ...
    def execute_buy(self, symbol, ticker, reason):
        """Execute buy - Fixed for Real Trading"""
        try:
            ...
            if amount_thb > max_amount:
                self.ai_log(f"⚠️ Limited to {max_amount} THB", "AI")
            ...
            if self.is_paper_trading:
                ...
                self.ai_log(f"🔴 REAL ORDER", "AI")

                # CRITICAL: Symbol must be THB_XXX format (uppercase)
                # From Scci5.py: symbol format is already correct
                buy_price = price * 1.002  # 0.2% buffer for slippage

                # Place REAL buy order using Bitkub API
                result = self.api_client.place_buy_order_safe(
                    symbol,  # Already in THB_XXX format
                    amount_thb,
                    buy_price,
                    'limit'
                )
                ...
                if result.get('error') != 0:
                    ...
                    self.ai_log(f"❌ FAILED: {error_msg}", "AI")

                    # Debug logging
                    print(f"\n❌ Real Buy Order Failed:")
                ...
                actual_amount = order_info.get('rec', amount_crypto)
                actual_fee = order_info.get('fee', buy_fee)
                ...
                self.strategy.position['buy_fee'] = actual_fee

                # Deduct from balance
                self.current_balance -= amount_thb

                print(f"\n✅ Real Buy Order Success:")
                print(f"   Order ID: {order_id}")
                print(f"   Amount: {actual_amount:.8f}")
                print(f"   Fee: {actual_fee:.2f} THB")
    ...
    def execute_sell(self, symbol, ticker, reason):
        """Execute sell - Fixed for Real Trading"""
        try:
            price = ticker['last_price']
            ...
            if self.is_paper_trading:
                ...
                # 🔥 REAL MODE - Actual Sell
                self.ai_log(f"🔴 REAL SELL", "AI")

                sell_price = price * 0.998  # 0.2% buffer
                ...
                if result.get('error') != 0:
                    ...
                    # Don't clear position - keep trying
                    self.visual.set_trading_state("HOLDING")
                    return
                ...
                self.ai_log(f"✅ Sold: {order_id}", "AI")

                # Recalculate with actual fee
                actual_net_pnl = gross_pnl - buy_fee - actual_fee
                ...
                print(f"\n✅ Real Sell Order Success:")
                print(f"   Order ID: {order_id}")
                print(f"   Gross P&L: {gross_pnl:+.2f} THB")
                print(f"   Total Fees: {buy_fee + actual_fee:.2f} THB")
