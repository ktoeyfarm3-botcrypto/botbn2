...
class EnhancedTradingBot:
    ...
    # ========================================================================
    # 🎯 แก้ไขขนาดกราฟใน setup_main_tab() method ของ Sci9.py
    # ========================================================================

    # ========================================================================
    # 🎬 แก้ไข Visual หายไป - ไม่มีปุ่ม Test เฉพาะ Visual อย่างเดียว
    # ========================================================================

    # 🎯 แทนที่ส่วน Visual ในท้าย setup_main_tab() ด้วยโค้ดนี้:

    def setup_main_tab(self):
        """Setup main dashboard tab with properly sized trading chart"""
        ...
        # Create main layout with balanced proportions
        ...
        self.main_log_display = ctk.CTkTextbox(log_frame, height=220)  # ปรับขนาดให้สมดุล
        self.main_log_display.pack(fill="both", expand=True, padx=10, pady=10)

        # Right side - Trading Chart + Visual System
        right_frame = ctk.CTkFrame(content_frame, width=380)
        right_frame.pack(side="right", fill="y", padx=(5, 0))
        right_frame.pack_propagate(False)

        # Chart header
        chart_header = ctk.CTkFrame(right_frame)
        chart_header.pack(fill="x", padx=8, pady=8)
        ...
        # Chart controls
        self.chart_symbol_combo = ctk.CTkComboBox(
            chart_header,
            values=["BTC_THB", "ETH_THB", "ADA_THB", "XRP_THB"],
            width=100,
            command=self.on_chart_symbol_change
        )
        self.chart_symbol_combo.pack(side="right", padx=3)
        self.chart_symbol_combo.set("BTC_THB")

        refresh_btn = ctk.CTkButton(
            chart_header,
            text="🔄",
            width=30,
            command=self.refresh_chart_data
        )
        refresh_btn.pack(side="right", padx=3)

        # Chart display area
        chart_frame = ctk.CTkFrame(right_frame, height=240)  # เพิ่มขนาดกราฟเล็กน้อย
        chart_frame.pack(fill="x", padx=8, pady=8)
        chart_frame.pack_propagate(False)

        # Initialize Chart System
        self.init_trading_chart(chart_frame)

        # =================================================================
        # 🎬 Sci-Fi Visual System - ไม่มีปุ่ม Test
        # =================================================================

        # Visual System section
        visual_section = ctk.CTkFrame(right_frame, height=330)  # ขนาดเหมาะสม
        visual_section.pack(fill="x", padx=10, pady=10)
        visual_section.pack_propagate(False)

        # Visual header (ไม่มีปุ่ม)
        visual_header = ctk.CTkFrame(visual_section)
        visual_header.pack(fill="x", padx=5, pady=5)

        ctk.CTkLabel(
            visual_header,
            text="🎬 System Status Visual",
            font=ctk.CTkFont(size=14, weight="bold")
        ).pack(pady=3)

        # Visual display area
        visual_display_frame = ctk.CTkFrame(visual_section, height=100)  # ความสูง 100px
        visual_display_frame.pack(fill="x", padx=5, pady=5)
        visual_display_frame.pack_propagate(False)

        # Initialize Sci-Fi Visual System with proper size
        self.scifi_visual = SciFiVisualSystem(visual_display_frame, width=350, height=95)

        # Quick price info (แสดงข้อมูลสำคัญ)
        info_frame = ctk.CTkFrame(visual_section, height=30)
        info_frame.pack(fill="x", padx=5, pady=(0, 5))
        info_frame.pack_propagate(False)

        # Price and volume in one line
        self.quick_price_label = ctk.CTkLabel(
            info_frame,
            text="BTC_THB: ฿0.00 (0.00%) | Vol: ฿0.00",
            font=ctk.CTkFont(size=11)
        )
        self.quick_price_label.pack(pady=5)

    # ========================================================================
    # 🔧 ปรับ methods เพื่อให้ Visual ทำงานอัตโนมัติ
    # ========================================================================

    # แก้ไข quick_connect_api() ให้มี visual feedback:
    def quick_connect_api(self):
        """Quick connect using saved credentials with visual feedback"""
        if self.api_key_entry.get() and self.secret_key_entry.get():
            # Set visual to connecting state
    ...
    # แก้ไข connect_api() ให้ update visual เมื่อเสร็จ:
    # ในส่วนท้าย method connect_api() เพิ่มบรรทัดนี้:
    def connect_api(self):
        """Connect to Bitkub API with visual feedback"""
        # ... โค้ดเดิมทั้งหมด ...

        # หาบรรทัดนี้ในฟังก์ชัน connect_thread():
        # self.root.after(0, self.update_status_displays)

        # เพิ่มหลังจากบรรทัดนั้น:
        # Update visual to success state
        if hasattr(self, 'scifi_visual') and self.scifi_visual:
            self.root.after(0, lambda: self.scifi_visual.set_state("success"))
            # Return to idle after 3 seconds
            self.root.after(3000, lambda: self.scifi_visual.set_state("idle"))

    # แก้ไข quick_start_trading():
    def quick_start_trading(self):
        """Quick start trading with visual feedback"""
        if not self.api_client:
            messagebox.showwarning("Warning", "Please connect to API first")
            self.notebook.set("🔧 Configuration")
            return

        # Set visual to analyzing state
        if hasattr(self, 'scifi_visual') and self.scifi_visual:
    ...
    # แก้ไข start_manual_trading():
    def start_manual_trading(self):
        """Start manual trading with visual feedback"""
        if not self.api_client:
            messagebox.showwarning("Warning", "Please connect to API first")
            return
        ...
        # Set visual to trading state
        if hasattr(self, 'scifi_visual') and self.scifi_visual:
            self.scifi_visual.set_state("trading")

        self.update_status_displays()

    # แก้ไข quick_stop_trading():
    def quick_stop_trading(self):
        """Quick stop trading with visual feedback"""
        self.stop_trading()

        # Set visual back to idle
        if hasattr(self, 'scifi_visual') and self.scifi_visual:
            self.scifi_visual.set_state("idle")

    # แก้ไข stop_trading():
    def stop_trading(self):
        """Stop all trading with visual feedback"""
        self.is_trading = False
        self.emergency_stop = False
    ...
    # แก้ไข on_chart_symbol_change():
    def on_chart_symbol_change(self, value):
        """Handle chart symbol change with visual feedback"""
        self.log_message(f"📊 Chart symbol changed to: {value}")

        # Set visual to analyzing briefly
        if hasattr(self, 'scifi_visual') and self.scifi_visual:
            self.scifi_visual.set_state("analyzing")
            ...
            def return_to_idle():
                if hasattr(self, 'scifi_visual') and self.scifi_visual:
                    current_state = "trading" if self.is_trading else "idle"
                    self.scifi_visual.set_state(current_state)

            import threading
            threading.Timer(2.0, return_to_idle).start()

        # Update chart immediately
        if hasattr(self, 'fig'):
            self.update_chart_display()
        elif hasattr(self, 'current_price_label'):
            self.update_simple_display()

    # แก้ไข update_simple_display() หรือ update_chart_display():
    # เพิ่มการอัพเดท quick price label:

    def update_chart_display(self):
        """Update chart with compact styling and price info"""
        # ... โค้ดเดิม ...

        # ในส่วนท้าย เพิ่ม:
        try:
            symbol = self.chart_symbol_combo.get().upper()
            if hasattr(self, 'quick_price_label') and market_data and market_data.get('error') == 0:
                data = market_data.get('result', {}).get(symbol, {})
                price = float(data.get('last', 0))
                change = float(data.get('percentChange', 0))
                volume = float(data.get('baseVolume', 0))

                change_symbol = "+" if change >= 0 else ""
                price_text = f"{symbol}: ฿{price:,.0f} ({change_symbol}{change:.2f}%) | Vol: ฿{volume:,.0f}"
                self.quick_price_label.configure(text=price_text)
        except:
            pass
    ...
    # ✅ ลบปุ่ม Test ออก
    # ✅ Visual ขนาด 95px height (เห็นชัดเจน)
    # ✅ Visual เปลี่ยนสถานะตามการทำงาน:
    #     - Connect API: connecting -> success -> idle
    #     - Start Trading: analyzing -> trading
    #     - Stop Trading: idle
    #     - Change Symbol: analyzing (2 วิ) -> กลับสถานะเดิม
    # ✅ Quick Price Info แสดงข้อมูลสำคัญ
    # ✅ Layout สมดุล: Chart 240px + Visual 140px

    # ========================================================================
    # 🎯 ขั้นตอนการแก้ไข:
    # ========================================================================

    # 1. แทนที่ส่วน setup_main_tab() ด้วยโค้ดใหม่ข้างต้น
    # 2. อัพเดท methods ที่เกี่ยวข้องให้มี visual feedback
    # 3. บันทึกและรันโปรแกรม
    # 4. ทดสอบโดยกดปุ่ม Connect API, Start/Stop Trading

    # ========================================================================
    # 🎬 ผลลัพธ์ที่ควรเห็น:
    # ========================================================================

    # ✅ Visual System แสดงผลชัดเจนขนาด 95px
    # ✅ Visual เปลี่ยนสีและแอนิเมชันตามการทำงาน
    # ✅ ไม่มีปุ่ม Test รบกวน
    # ✅ ข้อมูลราคาแสดงที่ด้านล่าง visual
    # ✅ Visual ทำงานอัตโนมัติตาม bot status

    def setup_matplotlib_chart(self):
        """Setup matplotlib chart with smaller size"""
        try:
            ...
            self.fig = Figure(figsize=(5.5, 3.5), dpi=70, facecolor='#2b2b2b')  # ลดขนาด
            self.ax = self.fig.add_subplot(111, facecolor='#1a1a1a')

            # Style the chart
            self.ax.tick_params(colors='white', labelsize=7)  # ลดขนาดตัวอักษร
            for spine in self.ax.spines.values():
                spine.set_color('white')
                spine.set_linewidth(0.5)

            # Create canvas
            self.chart_canvas = FigureCanvasTkAgg(self.fig, self.chart_container)
            ...
            self.log_message(f"❌ Chart setup error: {str(e)}")

    def setup_simple_chart_display(self, parent_frame):
        """Setup compact price display"""
        self.chart_container = ctk.CTkFrame(parent_frame)
        ...
        self.current_price_label.pack(pady=8)

        # Compact stats row
        stats_row = ctk.CTkFrame(self.chart_container)
        stats_row.pack(fill="x", padx=10, pady=5)

        self.change_24h_label = ctk.CTkLabel(
            stats_row,
            text="24h: 0.00%",
            font=ctk.CTkFont(size=11)
        )
        self.change_24h_label.pack(side="left", padx=5)
        ...
        # Compact high/low
        hl_row = ctk.CTkFrame(self.chart_container)
        hl_row.pack(fill="x", padx=10, pady=5)
        ...
        self.low_label.pack(side="right", padx=5)

        self.start_simple_chart_updates()

    def update_chart_display(self):
        """Update chart with compact styling"""
        if not hasattr(self, 'fig') or not self.api_client:
            return

        try:
            symbol = self.chart_symbol_combo.get().lower()
            market_data = self.api_client.get_market_data(symbol)

            if market_data and market_data.get('error') == 0:
                data = market_data.get('result', {}).get(symbol.upper(), {})
                base_price = float(data.get('last', 50000))
                ...
                # Generate 12 hours of data instead of 24 (more compact)
                times = [datetime.now() - timedelta(hours=12 - i) for i in range(12)]
                prices = []
                ...
                # Simple line chart
                self.ax.plot(times, prices, color='#00ff88', linewidth=1.5, label=symbol.upper())

                # Minimal styling
                self.ax.set_title(f'{symbol.upper()}', color='white', fontsize=11, pad=5)
                ...
                if hasattr(self, 'quick_price_label'):
                    change_symbol = "+" if percent_change >= 0 else ""
                    self.quick_price_label.configure(
                        text=f"Price: ฿{base_price:,.0f} ({change_symbol}{percent_change:.2f}%)"
                    )

                if hasattr(self, 'quick_volume_label'):
                    self.quick_volume_label.configure(text=f"Volume: ฿{volume:,.0f}")

        except Exception as e:
            self.log_message(f"❌ Chart update error: {str(e)}")

    def update_simple_display(self):
        """Update simple display with compact format"""
        if not self.api_client or not hasattr(self, 'current_price_label'):
            return
        ...
        try:
            ...
            if market_data and market_data.get('error') == 0:
                ...
                self.current_price_label.configure(text=f"Price: ฿{last_price:,.0f}")

                percent_change = float(data.get('percentChange', 0))
                change_color = "green" if percent_change >= 0 else "red"
                change_symbol = "+" if percent_change >= 0 else ""
                self.change_24h_label.configure(
                    text=f"24h: {change_symbol}{percent_change:.2f}%",
                    text_color=change_color
                )

                volume = float(data.get('baseVolume', 0))
                self.volume_label.configure(text=f"Vol: ฿{volume:,.0f}")

                high_24h = float(data.get('high24hr', 0))
                low_24h = float(data.get('low24hr', 0))
                self.high_label.configure(text=f"H: ฿{high_24h:,.0f}")
                self.low_label.configure(text=f"L: ฿{low_24h:,.0f}")
    ...
    # 📏 สรุปการปรับขนาด:
    # =================================================================
    # ✅ กราฟจาก 500px -> 380px width
    # ✅ ความสูงจาก auto -> 280px fixed
    # ✅ Figure size จาก (8,5) -> (5.5,3.5)
    # ✅ เพิ่ม Quick Info panel
    # ✅ ย่อข้อความและปุ่ม
    # ✅ Mini visual ขนาด 35px height
    # ✅ Layout ratio 60:40 แทน 50:50
...
# ============================================================================
    # ========================================================================
    # 🚨 แก้ไข Error: 'EnhancedTradingBot' object has no attribute 'on_chart_symbol_change'
    # ========================================================================

    # 🔍 สาเหตุ: Methods ใหม่ไม่ได้เพิ่มเข้าไปใน class EnhancedTradingBot
    # 🔧 วิธีแก้: เพิ่ม methods ทั้งหมดใน class EnhancedTradingBot

    # ========================================================================
    # 🎯 ขั้นตอนแก้ไข:
    # 1. หาบรรทัด class EnhancedTradingBot:
    # 2. หาตำแหน่งสุดท้ายของ class (ก่อน def main():)
    # 3. เพิ่ม methods ทั้งหมดข้างล่างนี้
    # ========================================================================

    # ===== เพิ่มข้างใน class EnhancedTradingBot: =====

    def init_trading_chart(self, parent_frame):
        """Initialize trading chart system"""
        try:
            ...
            # Fallback to simple text display if matplotlib not available
            self.log_message("⚠️ Matplotlib not available - using simple price display")
            self.setup_simple_chart_display(parent_frame)
    ...
    def setup_matplotlib_chart(self):
        ...
        try:
            ...
            # Create smaller figure with dark theme
            plt.style.use('dark_background')
            self.fig = Figure(figsize=(5.5, 3.5), dpi=70, facecolor='#2b2b2b')
            self.ax = self.fig.add_subplot(111, facecolor='#1a1a1a')
    ...
    def setup_simple_chart_display(self, parent_frame):
        """Setup compact price display when matplotlib not available"""
        self.chart_container = ctk.CTkFrame(parent_frame)
        self.chart_container.pack(fill="both", expand=True, padx=5, pady=5)
    ...
    def start_chart_updates(self):
        ...
        def update_loop():
            ...
            while True:
                try:
                    if hasattr(self, 'root') and hasattr(self, 'fig'):
    ...
    def start_simple_chart_updates(self):
        """Start updates for simple text display"""

        def simple_update_loop():
            import time
            while True:
                try:
                    if hasattr(self, 'root') and hasattr(self, 'current_price_label'):
                        if self.root.winfo_exists():
                            self.root.after(0, self.update_simple_display)
                            time.sleep(30)  # Update every 30 seconds
                        else:
                            break
                    else:
                    ...
                    print(f"Simple update loop error: {e}")
                    time.sleep(60)

        import threading
        threading.Thread(target=simple_update_loop, daemon=True).start()
    ...
    def refresh_chart_data(self):
        ...
        self.log_message("🔄 Refreshing chart data...")

        # Update chart based on available display type
        if hasattr(self, 'fig'):
            self.update_chart_display()
        elif hasattr(self, 'current_price_label'):
            self.update_simple_display()
...
# ========================================================================
# 📍 ตำแหน่งที่ต้องเพิ่ม methods ข้างต้น:
# ========================================================================

# หาใน Sci9.py:
# 1. บรรทัดที่มี: class EnhancedTradingBot:
# 2. เลื่อนไปล่างสุดของ class (ก่อน def main():)
# 3. เพิ่ม methods ทั้งหมดข้างต้นก่อน บรรทัด: def main():

# ========================================================================
# 🎯 วิธีหาตำแหน่งที่แน่นอน:
# ========================================================================

# กด Ctrl+F แล้วค้นหาสิ่งเหล่านี้ตามลำดับ:
# 1. "class EnhancedTradingBot:"
# 2. เลื่อนลงไปหา "def main():"
# 3. เพิ่ม methods ทั้งหมดก่อนบรรทัด "def main():"

# ========================================================================
# ✅ หลังจากเพิ่มแล้ว structure จะเป็นแบบนี้:
# ========================================================================
...
class EnhancedTradingBot:
    ...
    def refresh_chart_data(self):  # <-- methods ใหม่สุดท้าย
        # ... โค้ด ...
...
# ========================================================================
# 🔧 ถ้ายังไม่ทำงาน ให้ลองขั้นตอนนี้:
# ========================================================================

# 1. เปิด Sci9.py
# 2. กด Ctrl+F ค้นหา: "def main():"
# 3. ย้อนขึ้นไป 5-10 บรรทัด จะเห็นบรรทัดว่างๆ
# 4. เพิ่ม methods ทั้งหมดตรงนั้น
# 5. บันทึกไฟล์ แล้วลองรันใหม่

# ========================================================================
# 🚨 หมายเหตุสำคัญ:
# ========================================================================
# - ตรวจสอบ indentation ให้ถูกต้อง (ใช้ 4 spaces หรือ 1 tab)
# - methods ทั้งหมดต้องอยุ่ในระดับเดียวกันกับ methods อื่นๆ ใน class
# - ห้าม indent เกิน (จะกลายเป็น nested function)
