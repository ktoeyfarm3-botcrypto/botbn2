...
# Global variables for UI components (declared early)
auto_log_text = None
price_var = None
balance_var = None
pnl_var = None
...
# Function to update auto trading log (declared early)
def update_auto_log(message, log_type='AUTO'):
    """Update auto trading log display"""
    try:
        if auto_log_text is None:
            return

        auto_log_text.config(state="normal")

        timestamp = datetime.now().strftime("%H:%M:%S")

        # Insert new message at top
        auto_log_text.insert("1.0", f"[{timestamp}] {message}\n")

        # Keep only last 25 lines for better performance
        lines = auto_log_text.get("1.0", "end").split('\n')
        if len(lines) > 25:
            auto_log_text.delete(f"{len(lines) - 25}.0", "end")

        auto_log_text.config(state="disabled")
        auto_log_text.see("1.0")  # Scroll to top

    except Exception as e:
        print(f"Auto log update error: {e}")
...
price_frame = tk.Frame(status_content, bg="#2d3436")
price_frame.pack(fill="x")

price_var = tk.StringVar(value="📊 Loading...")
price_label = tk.Label(price_frame, textvariable=price_var,
                       font=("Consolas", 11, "bold"), fg="#00ff88", bg="#2d3436")
price_label.pack()

balance_frame = tk.Frame(status_content, bg="#2d3436")
balance_frame.pack(fill="x", pady=(5, 0))

balance_var = tk.StringVar(value="💰 Balance: Connecting...")
balance_label = tk.Label(balance_frame, textvariable=balance_var,
                         font=("Consolas", 10), fg="white", bg="#2d3436")
balance_label.pack()

pnl_frame = tk.Frame(status_content, bg="#2d3436")
pnl_frame.pack(fill="x", pady=(2, 0))

pnl_var = tk.StringVar(value="📊 PnL: Connecting...")
pnl_label = tk.Label(pnl_frame, textvariable=pnl_var,
                     font=("Consolas", 10), fg="white", bg="#2d3436")
pnl_label.pack()

# Add connection info
connection_frame = tk.Frame(status_content, bg="#2d3436")
connection_frame.pack(fill="x", pady=(5, 0))

connection_status = tk.StringVar(value="🔴 Disconnected")
connection_label = tk.Label(connection_frame, textvariable=connection_status,
                            fg="#74b9ff", bg="#2d3436", font=("Segoe UI", 9, "bold"))
...
# Emergency Controls Card (Fixed positioning)
...
# Make emergency button more prominent and always visible
emergency_main_btn = tk.Button(emergency_content, text="🚨 EMERGENCY STOP",
                               command=emergency_stop,
                               bg="#d63031", fg="white",
                               font=("Segoe UI", 12, "bold"),
                               relief="raised", bd=3, padx=20, pady=12,
                               cursor="hand2")
emergency_main_btn.pack(fill="x", pady=(0, 8))
...
# ===== Right Panel - Reduced size to accommodate left panel changes =====
log_panel = tk.Frame(main_container, bg="#0d1117", width=350)  # Reduced width
...
# Set up logger callback with auto log integration
def update_log_ui(log_entry):
    """Update log UI with new entry and auto log if relevant"""
    try:
        root.after(0, lambda: log_frame.add_log_entry(log_entry))

        # Also update auto log for relevant entries
        if log_entry['type'] in ['AUTO', 'SIGNAL', 'TRADE'] and auto_log_text is not None:
            root.after(0, lambda: update_auto_log(log_entry['message'], log_entry['type']))
...
# ===== Enhanced Status Bar =====
status_bar = tk.Frame(root, bg="#1f2937", height=50)  # Increased height
...
# Remove connection_status here as it's now in left panel
tk.Label(left_status, text="🚀 Trading Bot v2.1", fg="#74b9ff", bg="#1f2937",
...
def update_status_indicators():
    ...
    try:
        ...
        # Update safety info in auto card
        safety_info.config(text=f"Daily Trades: {bot.daily_trade_count}/{bot.max_daily_trades}")
