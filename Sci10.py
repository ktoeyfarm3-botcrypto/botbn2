...
class BLVCKTEAVisualSystem:
    """🖤 BLVCK TEA Futuristic Crypto Analysis Interface"""

    def __init__(self, parent_frame, width=600, height=500):
        ...
        # Create BLVCK TEA canvas
        self.canvas = tk.Canvas(
            parent_frame,
            width=width,
            height=height,
            bg="#0d0d0d",  # BLVCK background
            highlightthickness=0,
            relief='flat'
        )

        # BLVCK TEA color themes
        self.blvck_themes = {
            "idle": {
                "primary": "#00ffaa",  # Tea green
                "secondary": "#004433",  # Dark green
                "accent": "#66ffcc",  # Light green
                "glow": "#00cc88",  # Green glow
                "crypto": "#00ff99",  # Crypto green
                "data": "#33ffaa",  # Data green
                "text": "#ffffff",  # White text
                "panel": "#1a1a1a"  # Panel dark
            },
            "connecting": {
                "primary": "#ff9900",
                "secondary": "#664400",
        ...
        # Cryptocurrency data for analysis display (prices in THB)
        self.crypto_coins = [
            # Major coins (converted to THB approximately 1 USD = 36 THB)
            {"symbol": "BTC", "name": "Bitcoin", "price": 1630098.0, "change": 2.45, "status": "bullish"},
            {"symbol": "ETH", "name": "Ethereum", "price": 104050.8, "change": -1.23, "status": "neutral"},
            {"symbol": "BNB", "name": "Binance Coin", "price": 11368.8, "change": 3.67, "status": "bullish"},
            {"symbol": "ADA", "name": "Cardano", "price": 17.82, "change": -0.89, "status": "bearish"},
            {"symbol": "XRP", "name": "Ripple", "price": 18.83, "change": 1.23, "status": "neutral"},
            {"symbol": "SOL", "name": "Solana", "price": 3794.4, "change": 4.56, "status": "bullish"},
            {"symbol": "DOT", "name": "Polkadot", "price": 282.6, "change": -2.34, "status": "bearish"},
            {"symbol": "DOGE", "name": "Dogecoin", "price": 2.81, "change": 5.67, "status": "bullish"},
            {"symbol": "AVAX", "name": "Avalanche", "price": 1400.4, "change": -1.45, "status": "neutral"},
            {"symbol": "LINK", "name": "Chainlink", "price": 515.52, "change": 2.78, "status": "bullish"},
            {"symbol": "MATIC", "name": "Polygon", "price": 32.11, "change": -3.21, "status": "bearish"},
            {"symbol": "UNI", "name": "Uniswap", "price": 232.2, "change": 1.89, "status": "neutral"},
            {"symbol": "LTC", "name": "Litecoin", "price": 3330.0, "change": -0.67, "status": "neutral"},
            {"symbol": "BCH", "name": "Bitcoin Cash", "price": 8953.2, "change": 3.45, "status": "bullish"},
            {"symbol": "NEAR", "name": "NEAR Protocol", "price": 115.56, "change": -4.56, "status": "bearish"},
            {"symbol": "ATOM", "name": "Cosmos", "price": 448.2, "change": 2.34, "status": "bullish"},
            {"symbol": "FTM", "name": "Fantom", "price": 16.02, "change": 6.78, "status": "bullish"},
            {"symbol": "ALGO", "name": "Algorand", "price": 8.42, "change": -1.23, "status": "neutral"},
            {"symbol": "SAND", "name": "The Sandbox", "price": 20.41, "change": 4.32, "status": "bullish"},
            {"symbol": "MANA", "name": "Decentraland", "price": 24.41, "change": -2.56, "status": "bearish"}
        ]

        # Initialize BLVCK TEA systems first
        self.init_blvck_interface()

        # Currently analyzing coins (rotating selection)
        self.current_analysis = []
        self.analysis_rotation_timer = 0
        self.update_analysis_coins()

        # Initialize other systems
        self.init_crypto_nodes()
        self.init_analysis_streams()

        self.canvas.pack(pady=5)

    def init_blvck_interface(self):
        """Initialize BLVCK TEA interface elements"""
        # Main title area
        self.title_area = {
            'x': self.center_x, 'y': 25,
            'text': 'BLVCK TEA', 'subtitle': 'เครื่องวิเคราะห์คริปโต'
        }

        # Analysis panels - ปรับตำแหน่งให้ไม่บังส่วนสำคัญ
        self.analysis_panels = [
            {
                'x': 120, 'y': 100, 'width': 120, 'height': 70,  # ลดขนาดและปรับตำแหน่ง
                'title': 'กำลังสแกน', 'type': 'scan',
                'coins': [], 'update_timer': 0
            },
            {
                'x': 480, 'y': 100, 'width': 120, 'height': 70,  # เลื่อนไปขวาและลดขนาด
                'title': 'กำลังวิเคราะห์', 'type': 'analyze',
                'coins': [], 'update_timer': 0
            },
            {
                'x': 120, 'y': 350, 'width': 120, 'height': 70,  # ลดขนาดและปรับตำแหน่ง
                'title': 'สัญญาณ', 'type': 'signals',
                'coins': [], 'update_timer': 0
            },
            {
                'x': 480, 'y': 350, 'width': 120, 'height': 70,  # เลื่อนไปขวาและลดขนาด
                'title': 'กำลังติดตาม', 'type': 'monitor',
                'coins': [], 'update_timer': 0
            }
        ]

        # Central analysis display - ปรับให้ไม่บัง panels
        self.central_display = {
            'radius': 70, 'segments': 12,  # ลดขนาดเล็กน้อย
            'current_coin': None, 'rotation': 0
        }

    def init_crypto_nodes(self):
        """Initialize crypto analysis nodes"""
        self.crypto_nodes = []

        # Create nodes around central display - ปรับรัศมีให้ไม่บัง panels
        for i in range(8):
            angle = i * (2 * math.pi / 8)
            node = {
                'angle': angle,
                'radius': 110,  # ลดจาก 120
                'size': random.uniform(3, 6),  # ลดขนาด
                'activity': random.uniform(0.3, 1.0),
                'coin_index': random.randint(0, len(self.crypto_coins) - 1),
                'pulse_phase': random.uniform(0, 2 * math.pi),
                'analysis_type': random.choice(['price', 'volume', 'trend', 'signal'])
            }
            self.crypto_nodes.append(node)

    def init_analysis_streams(self):
        """Initialize data analysis streams"""
        self.analysis_streams = []

        # Create flowing data streams - ปรับให้ไม่บัง panels
        for i in range(4):  # ลดจาก 6
            stream = {
                'id': i,
                'points': [],
                'data_type': random.choice(['price_data', 'volume_flow', 'trend_analysis', 'signal_processing']),
                'flow_speed': random.uniform(0.1, 0.3),
                'intensity': random.uniform(0.4, 0.8)  # ลด intensity
            }

            # Generate stream path - ปรับเส้นทางให้หลีกเลี่ยง panels
            for j in range(15):  # ลดจาก 20
                t = j / 14
                angle = i * (2 * math.pi / 4) + t * math.pi * 1.5
                radius = 50 + t * 35  # ลดรัศมี

                x = math.cos(angle) * radius
                y = math.sin(angle) * radius

                stream['points'].append({'x': x, 'y': y, 'intensity': t * 0.6})

            self.analysis_streams.append(stream)

    def update_analysis_coins(self):
        """Update coins being analyzed"""
        # Check if analysis_panels exists first
        if not hasattr(self, 'analysis_panels'):
            return

        # Rotate selection of coins for different panels
        self.current_analysis = random.sample(self.crypto_coins, min(12, len(self.crypto_coins)))

        # Assign coins to panels
        for i, panel in enumerate(self.analysis_panels):
            start_idx = i * 3
            end_idx = (i + 1) * 3
            panel['coins'] = self.current_analysis[start_idx:end_idx] if start_idx < len(self.current_analysis) else []

    def start_animation(self, state="idle"):
        """Start BLVCK TEA animation"""
        self.current_state = state
        if not self.is_animating:
            self.is_animating = True
            self.animation_thread = threading.Thread(target=self._blvck_animation_loop, daemon=True)
    ...
    def stop_animation(self):
        ...
        self.draw_static_blvck()

    def set_state(self, state):
        """Change BLVCK TEA state"""
        if state in self.blvck_themes:
            if state != self.current_state:
                self.trigger_blvck_transition(state)
            self.current_state = state
            if state != "idle":
    ...
    def trigger_blvck_transition(self, new_state):
        """BLVCK TEA transition effect"""
        threading.Thread(target=self._blvck_transition, args=(new_state,), daemon=True).start()

    def _blvck_transition(self, new_state):
        """Execute BLVCK TEA transition"""
        theme = self.blvck_themes[new_state]

        # Digital matrix transition
        for wave in range(10):
            alpha = 1 - (wave / 10)
            color = self.blend_color(theme["primary"], "#0d0d0d", alpha * 0.6)

            # Create expanding hexagon
            points = []
            for angle in range(0, 360, 60):
                x = self.center_x + math.cos(math.radians(angle)) * wave * 15
                y = self.center_y + math.sin(math.radians(angle)) * wave * 15
                points.extend([x, y])

            if len(points) >= 6:
                self.canvas.create_polygon(
                    points, fill="", outline=color,
                    width=max(1, int(3 * alpha)), tags="blvck_transition"
                )

            self.canvas.update()
            time.sleep(0.05)

        self.canvas.delete("blvck_transition")

    def _blvck_animation_loop(self):
        """BLVCK TEA animation loop - Ultra Smooth 120+ FPS"""
        while self.is_animating:
            try:
                # Update phases - Ultra smooth increments
                self.frame_count += 1
                self.time_phase += 0.008  # Very small increments for smoothness
                self.crypto_phase += 0.012
                self.analysis_phase += 0.01
                self.analysis_rotation_timer += 1

                # Update coin analysis every 1200 frames (10 seconds at 120fps)
                if self.analysis_rotation_timer % 1200 == 0:
                    self.update_analysis_coins()
                ...
                # Get current theme
                theme = self.blvck_themes.get(self.current_state, self.blvck_themes["idle"])

                # Draw BLVCK TEA layers
                self.draw_blvck_background_smooth(theme)
                self.draw_title_display_smooth(theme)
                self.draw_analysis_panels_smooth(theme)
                self.draw_central_scanner_smooth(theme)
                self.draw_crypto_nodes_smooth(theme)
                self.draw_analysis_streams_smooth(theme)
                self.draw_status_bar_smooth(theme)

                # State-specific effects - smooth versions
                if self.current_state == "analyzing":
                    self.draw_deep_analysis_smooth(theme)
                elif self.current_state == "trading":
                    self.draw_trading_execution_smooth(theme)
                elif self.current_state in ["buy_signal", "sell_signal"]:
                    self.draw_signal_alert_smooth(theme)

                # Update systems smoothly
                self.update_blvck_systems_smooth()

                time.sleep(0.008)  # ~120-125 FPS for ultra smooth

            except Exception as e:
                print(f"BLVCK TEA Animation error: {e}")
                break

    def draw_blvck_background_smooth(self, theme):
        """Draw BLVCK TEA background matrix - Smooth version"""
        # Digital grid - much more subtle
        grid_alpha = 0.05 + 0.02 * math.sin(self.time_phase * 0.3)

        # Less frequent vertical lines
        for x in range(0, self.width + 1, 60):
            color = self.blend_color(theme["accent"], "#0d0d0d", grid_alpha)
            self.canvas.create_line(
                x, 0, x, self.height,
                fill=color, width=1, tags="grid"
            )

        # Less frequent horizontal lines
        for y in range(0, self.height + 1, 60):
            color = self.blend_color(theme["accent"], "#0d0d0d", grid_alpha)
            self.canvas.create_line(
                0, y, self.width, y,
                fill=color, width=1, tags="grid"
        ...
        # Reduced digital noise particles
        for _ in range(8):  # ลดจาก 20
            x = random.randint(0, self.width)
            y = random.randint(0, self.height)
            size = random.uniform(0.5, 1.5)
            alpha = random.uniform(0.1, 0.3)

            color = self.blend_color(theme["data"], "#0d0d0d", alpha)
            self.canvas.create_oval(
                x - size, y - size, x + size, y + size,
                fill=color, outline="", tags="noise"
            )

    def draw_title_display_smooth(self, theme):
        """Draw BLVCK TEA title - Smooth version with better positioning"""
        title = self.title_area

        # Main title with gentle pulse
        title_pulse = 0.85 + 0.15 * math.sin(self.time_phase * 0.8)
        title_color = self.blend_color(theme["primary"], "#0d0d0d", title_pulse)

        # Move title higher to avoid being covered
        title_y = 20  # ลดจาก 25

        self.canvas.create_text(
            title['x'], title_y,
            text=title['text'], fill=title_color,
            font=("Orbitron", 16, "bold"), tags="main_title"
        )

        # Subtitle
        subtitle_color = self.blend_color(theme["accent"], "#0d0d0d", 0.7)
        self.canvas.create_text(
            title['x'], title_y + 18,  # ปรับตำแหน่ง
            text=title['subtitle'], fill=subtitle_color,
            font=("Orbitron", 8), tags="subtitle"
        )

        # Title underline - more subtle
        underline_width = 100
        underline_color = self.blend_color(theme["glow"], "#0d0d0d", title_pulse * 0.4)
        self.canvas.create_line(
            title['x'] - underline_width // 2, title_y + 28,
            title['x'] + underline_width // 2, title_y + 28,
            fill=underline_color, width=1, tags="title_underline"
        )

    def draw_analysis_panels_smooth(self, theme):
        """Draw crypto analysis panels - Compact version"""
        for panel in self.analysis_panels:
            # Panel background - very subtle pulse
            panel_pulse = 0.15 + 0.05 * math.sin(self.analysis_phase + panel['x'] * 0.005)
            bg_color = self.blend_color(theme["panel"], "#0d0d0d", panel_pulse)
            border_color = self.blend_color(theme["accent"], "#0d0d0d", 0.4)
            ...
            # Panel title - ลดขนาดฟอนต์
            title_color = self.blend_color(theme["primary"], "#0d0d0d", 0.8)
            self.canvas.create_text(
                panel['x'], panel['y'] - panel['height'] // 2 + 12,  # ปรับตำแหน่ง
                text=panel['title'], fill=title_color,
                font=("Orbitron", 8, "bold"), tags="panel_title"  # ลดขนาดฟอนต์
            )

            # Coin data display - compact version
            if 'coins' in panel and panel['coins']:
                for i, coin in enumerate(panel['coins'][:2]):  # แสดงแค่ 2 เหรียญแทน 3
                    coin_y = panel['y'] - panel['height'] // 2 + 28 + i * 14

                    # Coin symbol
                    symbol_color = self.get_coin_color(coin, theme)
                    self.canvas.create_text(
                        panel['x'] - 35, coin_y,
                        text=coin['symbol'], fill=symbol_color,
                        font=("Consolas", 7, "bold"), tags="coin_symbol"  # ลดขนาดฟอนต์
                    )

                    # Price change indicator
                    change_color = theme["crypto"] if coin['change'] > 0 else theme["accent"]
                    if coin['change'] > 0:
                        indicator = "▲"
                    elif coin['change'] < 0:
                        indicator = "▼"
                    ...
                    self.canvas.create_text(
                        panel['x'] + 8, coin_y,
                        text=indicator, fill=change_color,
                        font=("Consolas", 7), tags="change_indicator"
                    )

                    # Price - compact
                    price_text = self.format_price_compact(coin['price'])
                    price_color = self.blend_color(theme["data"], "#0d0d0d", 0.7)
                    self.canvas.create_text(
                        panel['x'] + 30, coin_y,
                        text=price_text, fill=price_color,
                        font=("Consolas", 6), tags="coin_price"  # ลดขนาดฟอนต์
                    )

    def format_price_compact(self, price):
    ...
    def draw_central_scanner_smooth(self, theme):
        """Draw central analysis scanner - Smooth version"""
        scanner = self.central_display
        scanner['rotation'] += 0.5  # ลดความเร็วลง

        # Scanner ring - smoother segments
        ring_segments = scanner['segments']
        for i in range(ring_segments):
            if i % 4 == 0:  # แสดงน้อยลง
                continue

            angle = (i * (360 / ring_segments) + scanner['rotation']) % 360
            start_angle = math.radians(angle - 8)
            end_angle = math.radians(angle + 8)

            x1 = self.center_x + math.cos(start_angle) * scanner['radius']
            y1 = self.center_y + math.sin(start_angle) * scanner['radius']
            x2 = self.center_x + math.cos(end_angle) * scanner['radius']
            y2 = self.center_y + math.sin(end_angle) * scanner['radius']

            segment_intensity = 0.5 + 0.3 * math.sin(self.crypto_phase + i * 0.3)
            color = self.blend_color(theme["primary"], "#0d0d0d", segment_intensity)

            self.canvas.create_line(
                x1, y1, x2, y2,
                fill=color, width=2, capstyle=tk.ROUND, tags="scanner_ring"
            )

        # Central coin display - stable
        if self.current_analysis and len(self.current_analysis) > 0:
            coin_index = self.frame_count // 120 % len(self.current_analysis)  # เปลี่ยนช้าลง
            current_coin = self.current_analysis[coin_index]

            # Coin symbol
            coin_color = self.get_coin_color(current_coin, theme)
            self.canvas.create_text(
                self.center_x, self.center_y - 10,
                text=current_coin['symbol'], fill=coin_color,
                font=("Orbitron", 14, "bold"), tags="central_coin"
            )

            # Price
            price_text = self.format_price(current_coin['price'])
            price_color = self.blend_color(theme["data"], "#0d0d0d", 0.8)
            self.canvas.create_text(
                self.center_x, self.center_y + 8,
                text=price_text, fill=price_color,
                font=("Consolas", 10), tags="central_price"
            )

    def draw_crypto_nodes_smooth(self, theme):
        """Draw crypto analysis nodes - Smooth version"""
        for node in self.crypto_nodes:
            node_angle = node['angle'] + self.crypto_phase * 0.3  # ลดความเร็ว
            x = self.center_x + math.cos(node_angle) * node['radius']
            y = self.center_y + math.sin(node_angle) * node['radius']

            # Node activity - smoother
            activity = 0.6 + 0.2 * math.sin(node['pulse_phase'] + self.time_phase)
            size = node['size'] * (0.9 + 0.2 * activity)

            # Node color based on analysis type
            node_colors = {
                'price': theme["primary"],
                'volume': theme["crypto"],
                'trend': theme["data"],
            ...
            node_color = node_colors.get(node['analysis_type'], theme["accent"])

            # Node glow - subtle
            glow_size = size * 1.3
            glow_color = self.blend_color(node_color, "#0d0d0d", activity * 0.2)
            self.canvas.create_oval(
                x - glow_size, y - glow_size,
                x + glow_size, y + glow_size,
                fill=glow_color, outline="", tags="node_glow"
            )

            # Main node
            main_color = self.blend_color(node_color, "#0d0d0d", activity * 0.6)
            self.canvas.create_oval(
                x - size, y - size,
                x + size, y + size,
                fill=main_color, outline="", tags="crypto_node"
            )

            # Connection to center - only when very active
            if activity > 0.8:
                connection_color = self.blend_color(node_color, "#0d0d0d", activity * 0.3)
                self.canvas.create_line(
                    self.center_x, self.center_y, x, y,
                    fill=connection_color, width=1, tags="node_connection"
                )

    def draw_analysis_streams_smooth(self, theme):
        """Draw data analysis streams - Smooth version"""
        for stream in self.analysis_streams:
            points = stream['points']
            stream_colors = {
                'price_data': theme["primary"],
                'volume_flow': theme["crypto"],
                'trend_analysis': theme["data"],
                'signal_processing': theme["glow"]
            }
            stream_color = stream_colors.get(stream['data_type'], theme["accent"])

            # Draw stream flow - smoother
            for i in range(len(points) - 1):
                p1, p2 = points[i], points[i + 1]

                x1 = self.center_x + p1['x']
                y1 = self.center_y + p1['y']
                x2 = self.center_x + p2['x']
                y2 = self.center_y + p2['y']

                # Flow effect - smoother
                flow = 0.5 + 0.3 * math.sin(self.analysis_phase * 2 + i * 0.2)
                alpha = p1['intensity'] * flow * stream['intensity'] * 0.6

                if alpha > 0.2:
                    color = self.blend_color(stream_color, "#0d0d0d", alpha)
                    width = max(1, int(1.5 * alpha))

                    self.canvas.create_line(
                        x1, y1, x2, y2,
                        fill=color, width=width, capstyle=tk.ROUND,
                        tags="analysis_stream"
                    )

    def draw_status_bar_smooth(self, theme):
        """Draw status bar with crypto info - Smooth version"""
        status_texts = {
            "idle": "◦ BLVCK TEA - เครื่องสแกนคริปโตพร้อมใช้งาน ◦",
            "connecting": "◦ กำลังเชื่อมต่อกับตลาดคริปโต ◦",
            "analyzing": "◦ กำลังวิเคราะห์คริปโตเชิงลึก ◦",
            "buy_signal": "◦ ตรวจพบสัญญาณซื้อ - แนวโน้มขาขึ้น ◦",
            "sell_signal": "◦ ตรวจพบสัญญาณขาย - แนวโน้มขาลง ◦",
            "trading": "◦ กำลังดำเนินการเทรดคริปโต ◦",
            "success": "◦ เทรดสำเร็จ - ได้กำไรแล้ว ◦",
            "error": "◦ ระบบเกิดข้อผิดพลาด - กำลังเชื่อมต่อใหม่ ◦"
        }

        status = status_texts.get(self.current_state, "◦ BLVCK TEA สถานะไม่ทราบ ◦")

        # Status text with gentle pulse
        status_pulse = 0.8 + 0.1 * math.sin(self.time_phase * 1.2)
        status_color = self.blend_color(theme["primary"], "#0d0d0d", status_pulse)

        self.canvas.create_text(
            self.center_x, self.height - 20,  # เลื่อนขึ้นเล็กน้อย
            text=status, fill=status_color,
            font=("Orbitron", 9, "bold"), tags="status_bar"
        )

    def draw_deep_analysis_smooth(self, theme):
        """Draw deep analysis scanning effect - Smooth version"""
        scan_radius = 100 + 20 * math.sin(self.analysis_phase * 1.2)
        scan_intensity = 0.6 + 0.2 * math.cos(self.analysis_phase * 1.5)

        # Analysis scan rings - subtle
        for ring in range(3):  # ลดจาก 4
            radius = scan_radius + ring * 25
            alpha = scan_intensity * (1 - ring * 0.25) * 0.5

            if alpha > 0.1:
                color = self.blend_color(theme["crypto"], "#0d0d0d", alpha)

                self.canvas.create_oval(
                    self.center_x - radius, self.center_y - radius,
                    self.center_x + radius, self.center_y + radius,
                    outline=color, width=max(1, int(2 * alpha)),
                    tags="deep_analysis"
                )

    def draw_trading_execution_smooth(self, theme):
        """Draw trading execution effects - Smooth version"""
        execution_intensity = 0.5 + 0.2 * math.sin(self.time_phase * 2.5)

        # Trading bursts - fewer and smoother
        for i in range(8):  # ลดจาก 12
            angle = i * 45 + self.time_phase * 30
            burst_length = 60 * execution_intensity

            end_x = self.center_x + math.cos(math.radians(angle)) * burst_length
            end_y = self.center_y + math.sin(math.radians(angle)) * burst_length

            color = self.blend_color(theme["glow"], "#0d0d0d", execution_intensity * 0.6)

            self.canvas.create_line(
                self.center_x, self.center_y, end_x, end_y,
                fill=color, width=max(1, int(2 * execution_intensity)),
                tags="trading_burst"
            )

    def draw_signal_alert_smooth(self, theme):
        """Draw signal alert effects - Smooth version"""
        alert_pulse = 0.4 + 0.3 * math.sin(self.time_phase * 3.5)

        # Alert rings - subtle
        for ring in range(2):  # ลดจาก 3
            radius = 80 + ring * 30 + alert_pulse * 10
            alpha = alert_pulse * (1 - ring * 0.3) * 0.6

            color = self.blend_color(theme["primary"], "#0d0d0d", alpha)
    ...
    def update_blvck_systems_smooth(self):
        """Update all BLVCK TEA systems - Smooth version"""
        # Update crypto nodes - less frequent
        for node in self.crypto_nodes:
            node['pulse_phase'] += random.uniform(0.005, 0.02)  # ลดลง
            if random.random() < 0.01:  # ลดจาก 0.05
                node['coin_index'] = random.randint(0, len(self.crypto_coins) - 1)
                node['activity'] = random.uniform(0.4, 0.8)  # ลด range

        # Update coin prices - less frequent
        for coin in self.crypto_coins:
            if random.random() < 0.005:  # ลดจาก 0.02
                price_change = random.uniform(-0.02, 0.02)  # ลดการเปลี่ยนแปลง
                old_price = coin['price']
                coin['price'] *= (1 + price_change)
                coin['change'] = price_change * 100

                if coin['price'] < 0.1:
                    coin['price'] = 0.1

    def draw_title_display(self, theme):
        """Draw BLVCK TEA title"""
        title = self.title_area

        # Main title with glow
        title_pulse = 0.8 + 0.2 * math.sin(self.time_phase * 1.2)
        title_color = self.blend_color(theme["primary"], "#0d0d0d", title_pulse)
        ...
        # Subtitle
        subtitle_color = self.blend_color(theme["accent"], "#0d0d0d", 0.8)
        self.canvas.create_text(
            title['x'], title['y'] + 20,
            text=title['subtitle'], fill=subtitle_color,
            font=("Orbitron", 8), tags="subtitle"
        ...
        # Title underline
        underline_width = 120
        underline_color = self.blend_color(theme["glow"], "#0d0d0d", title_pulse * 0.6)
        self.canvas.create_line(
            title['x'] - underline_width // 2, title['y'] + 30,
            title['x'] + underline_width // 2, title['y'] + 30,
            fill=underline_color, width=2, tags="title_underline"
        )

    def draw_analysis_panels(self, theme):
        """Draw crypto analysis panels"""
        for panel in self.analysis_panels:
            # Panel background
            panel_pulse = 0.3 + 0.1 * math.sin(self.analysis_phase + panel['x'] * 0.01)
            bg_color = self.blend_color(theme["panel"], "#0d0d0d", panel_pulse)
            border_color = self.blend_color(theme["accent"], "#0d0d0d", 0.6)
            ...
            # Panel title
            title_color = self.blend_color(theme["primary"], "#0d0d0d", 0.9)
            self.canvas.create_text(
                panel['x'], panel['y'] - panel['height'] // 2 + 15,
                text=panel['title'], fill=title_color,
                font=("Orbitron", 10, "bold"), tags="panel_title"
            )

            # Coin data display
            if 'coins' in panel and panel['coins']:
                for i, coin in enumerate(panel['coins'][:3]):  # Show max 3 coins
                    coin_y = panel['y'] - panel['height'] // 2 + 35 + i * 15
                    ...
                    self.canvas.create_text(
                        panel['x'] - 40, coin_y,
                        text=coin['symbol'], fill=symbol_color,
                        font=("Consolas", 8, "bold"), tags="coin_symbol"
                    )
                    ...
                    self.canvas.create_text(
                        panel['x'] + 10, coin_y,
                        text=indicator, fill=change_color,
                        font=("Consolas", 8), tags="change_indicator"
                    )

                    # Price
                    price_text = self.format_price(coin['price'])
                    price_color = self.blend_color(theme["data"], "#0d0d0d", 0.8)
                    self.canvas.create_text(
                        panel['x'] + 35, coin_y,
                        text=price_text, fill=price_color,
                        font=("Consolas", 7), tags="coin_price"
                    )

    def draw_central_scanner(self, theme):
        """Draw central analysis scanner"""
        scanner = self.central_display
        scanner['rotation'] += 2
        ...
        ring_segments = scanner['segments']
        for i in range(ring_segments):
            if i % 3 == 0:  # Skip segments for scanner effect
                continue
            ...
            segment_intensity = 0.6 + 0.4 * math.sin(self.crypto_phase + i * 0.5)
            color = self.blend_color(theme["primary"], "#0d0d0d", segment_intensity)

            self.canvas.create_line(
                x1, y1, x2, y2,
                fill=color, width=3, capstyle=tk.ROUND, tags="scanner_ring"
            )

        # Central coin display
        if self.current_analysis and len(self.current_analysis) > 0:
            coin_index = self.frame_count // 60 % len(self.current_analysis)
            current_coin = self.current_analysis[coin_index]
            ...
            # Price
            price_text = self.format_price(current_coin['price'])
            price_color = self.blend_color(theme["data"], "#0d0d0d", 0.9)
            self.canvas.create_text(
                self.center_x, self.center_y + 8,
                text=price_text, fill=price_color,
                font=("Consolas", 10), tags="central_price"
    ...
    def draw_crypto_nodes(self, theme):
        """Draw crypto analysis nodes"""
        for node in self.crypto_nodes:
            node_angle = node['angle'] + self.crypto_phase * 0.5
            x = self.center_x + math.cos(node_angle) * node['radius']
            y = self.center_y + math.sin(node_angle) * node['radius']

            # Node activity
            activity = 0.6 + 0.4 * math.sin(node['pulse_phase'] + self.time_phase)
            size = node['size'] * (0.8 + 0.4 * activity)
            ...
            # Node glow
            glow_size = size * 1.5
            glow_color = self.blend_color(node_color, "#0d0d0d", activity * 0.3)
            self.canvas.create_oval(
                x - glow_size, y - glow_size,
                x + glow_size, y + glow_size,
                fill=glow_color, outline="", tags="node_glow"
            ...
            # Main node
            main_color = self.blend_color(node_color, "#0d0d0d", activity * 0.8)
            self.canvas.create_oval(
                x - size, y - size,
                x + size, y + size,
                fill=main_color, outline="", tags="crypto_node"
            ...
            if activity > 0.7:
                connection_color = self.blend_color(node_color, "#0d0d0d", activity * 0.4)
                self.canvas.create_line(
                    self.center_x, self.center_y, x, y,
                    fill=connection_color, width=1, tags="node_connection"
                )

    def draw_analysis_streams(self, theme):
        """Draw data analysis streams"""
        for stream in self.analysis_streams:
            points = stream['points']
            stream_colors = {
                'price_data': theme["primary"],
            ...
            for i in range(len(points) - 1):
                ...
                # Flow effect
                flow = 0.4 + 0.6 * math.sin(self.analysis_phase * 3 + i * 0.3)
                alpha = p1['intensity'] * flow * stream['intensity']
    ...
    def draw_deep_analysis(self, theme):
        """Draw deep analysis scanning effect"""
        scan_radius = 100 + 30 * math.sin(self.analysis_phase * 1.5)
        scan_intensity = 0.8 + 0.2 * math.cos(self.analysis_phase * 2)

        # Analysis scan rings
        for ring in range(4):
            radius = scan_radius + ring * 20
            alpha = scan_intensity * (1 - ring * 0.2)
    ...
    def draw_trading_execution(self, theme):
        """Draw trading execution effects"""
        execution_intensity = 0.7 + 0.3 * math.sin(self.time_phase * 3)

        # Trading bursts
        for i in range(12):
            angle = i * 30 + self.time_phase * 45
            burst_length = 80 * execution_intensity

            end_x = self.center_x + math.cos(math.radians(angle)) * burst_length
            end_y = self.center_y + math.sin(math.radians(angle)) * burst_length

            color = self.blend_color(theme["glow"], "#0d0d0d", execution_intensity)

            self.canvas.create_line(
                self.center_x, self.center_y, end_x, end_y,
                fill=color, width=max(1, int(3 * execution_intensity)),
                tags="trading_burst"
            )

    def draw_signal_alert(self, theme):
        """Draw signal alert effects"""
        alert_pulse = 0.5 + 0.5 * math.sin(self.time_phase * 4)

        # Alert rings
        for ring in range(3):
            radius = 90 + ring * 25 + alert_pulse * 15
            alpha = alert_pulse * (1 - ring * 0.25)
            ...
            self.canvas.create_oval(
                self.center_x - radius, self.center_y - radius,
                self.center_x + radius, self.center_y + radius,
                outline=color, width=max(1, int(4 * alpha)),
                tags="signal_alert"
            )

    def draw_status_bar(self, theme):
        """Draw status bar with crypto info"""
        status_texts = {
            "idle": "◦ BLVCK TEA - เครื่องสแกนคริปโตพร้อมใช้งาน ◦",
            "connecting": "◦ กำลังเชื่อมต่อกับตลาดคริปโต ◦",
            "analyzing": "◦ กำลังวิเคราะห์คริปโตเชิงลึก ◦",
        ...
        status = status_texts.get(self.current_state, "◦ BLVCK TEA STATUS UNKNOWN ◦")

        # Status text with pulse
        status_pulse = 0.8 + 0.2 * math.sin(self.time_phase * 1.5)
        status_color = self.blend_color(theme["primary"], "#0d0d0d", status_pulse)

        self.canvas.create_text(
            self.center_x, self.height - 25,
            text=status, fill=status_color,
            font=("Orbitron", 10, "bold"), tags="status_bar"
        )

    def get_coin_color(self, coin, theme):
        """Get color for coin based on status"""
        status_colors = {
            'bullish': theme["crypto"],
            'bearish': theme["accent"],
            'neutral': theme["data"]
        }
        return status_colors.get(coin['status'], theme["text"])

    def format_price(self, price):
        """Format price for display in Thai Baht (฿)"""
        if price >= 1000000:
            # เหรียญราคาสูงมาก (เช่น BTC) แสดงเป็น ล้านบาท
            return f"฿{price / 1000000:.2f}M"
        elif price >= 1000:
            # เหรียญราคาปานกลาง แสดงเป็น พันบาท
            return f"฿{price:,.0f}"
        elif price >= 100:
            # เหรียญราคา 100+ บาท
            return f"฿{price:.0f}"
            ...
            # เหรียญราคาต่ำกว่า 1 บาท
            return f"฿{price:.3f}"

    def update_blvck_systems(self):
        """Update all BLVCK TEA systems"""
        # Update crypto nodes
        for node in self.crypto_nodes:
            node['pulse_phase'] += random.uniform(0.02, 0.08)
            if random.random() < 0.05:
                node['coin_index'] = random.randint(0, len(self.crypto_coins) - 1)
                node['activity'] = random.uniform(0.3, 1.0)

        # Update coin prices (simulation in THB)
        for coin in self.crypto_coins:
            if random.random() < 0.02:  # 2% chance to update price
                price_change = random.uniform(-0.05, 0.05)  # ±5% change
                old_price = coin['price']
                coin['price'] *= (1 + price_change)
                coin['change'] = price_change * 100

                # Ensure minimum prices in THB
                if coin['price'] < 0.1:
                    coin['price'] = 0.1

    def blend_color(self, color1: str, color2: str, alpha: float) -> str:
        """Blend two colors"""
        try:
    ...
    def draw_static_blvck(self):
        """Draw static BLVCK TEA display"""
        self.canvas.delete("all")
        theme = self.blvck_themes["idle"]

        # Static title
        self.canvas.create_text(
            self.center_x, 25,
            text="BLVCK TEA", fill=theme["primary"],
            font=("Orbitron", 16, "bold"), tags="static_title"
        )

        self.canvas.create_text(
            self.center_x, 45,
            text="CRYPTO ANALYSIS ENGINE", fill=theme["accent"],
            font=("Orbitron", 8), tags="static_subtitle"
        )
        ...
        self.canvas.create_oval(
            ...
            self.center_x + 80, self.center_y + 80,
            outline=theme["primary"], width=3, tags="static_scanner"
        )

        # Static status
        self.canvas.create_text(
            self.center_x, self.height - 25,
            text="◦ BLVCK TEA เครื่องสแกนคริปโตพร้อมใช้งาน ◦",
            fill=theme["primary"],
            font=("Orbitron", 10, "bold"), tags="static_status"
        )

    def crypto_pulse_effect(self):
        """Special crypto pulse effect"""
        threading.Thread(target=self._crypto_pulse, daemon=True).start()

    def _crypto_pulse(self):
        """Execute crypto pulse effect"""
        theme = self.blvck_themes[self.current_state]

        for pulse in range(15):
            radius = pulse * 20
            alpha = 1 - (pulse / 15)

            color = self.blend_color(theme["crypto"], "#0d0d0d", alpha * 0.8)

            self.canvas.create_oval(
                self.center_x - radius, self.center_y - radius,
                self.center_x + radius, self.center_y + radius,
                outline=color, width=max(1, int(4 * alpha)),
                tags="crypto_pulse_effect"
            )

            self.canvas.update()
            time.sleep(0.04)

        self.canvas.delete("crypto_pulse_effect")

    def cleanup(self):
        """Cleanup BLVCK TEA resources"""
        self.stop_animation()
...
# ============================================================================
# 🖤 Integration Guide for Sci9.py
# ============================================================================

def integrate_blvck_tea_into_sci9():
    """
    วิธีการใช้ BLVCK TEA ใน Sci9.py:

    1. แทนที่ Visual System:
        # เดิม:
        self.scifi_visual = FuturisticUltraVisual(visual_frame)

        # ใหม่:
        self.scifi_visual = BLVCKTEAVisualSystem(visual_frame, width=600, height=500)

    2. ปรับขนาด Frame (เหมาะกับ Sci9.py):
        right_frame = ctk.CTkFrame(content_frame, width=650)
        right_frame.pack(side="right", fill="y", padx=(10, 0))

    3. เพิ่มปุ่มควบคุม BLVCK TEA:
        visual_states = [
            ("🖤 Idle", "idle"),
    ...
    4. เพิ่ม BLVCK TEA Special Effects:
        # Crypto Pulse Button
        pulse_btn = ctk.CTkButton(
            visual_controls_frame,
            text="🪙 Crypto Pulse",
            command=lambda: self.scifi_visual.crypto_pulse_effect(),
            fg_color="#004433",
            hover_color="#00aa66",
            width=120, height=30
        )
        pulse_btn.pack(pady=2)

    5. ปรับธีมแอพให้เข้ากับ BLVCK TEA:
        # เปลี่ยนสีพื้นหลังหลัก
        self.root.configure(bg="#0d0d0d")

        # ปรับสี frame ต่างๆ
        main_frame.configure(fg_color="#1a1a1a")
    """
    pass


# Test BLVCK TEA System
if __name__ == "__main__":
    root = tk.Tk()
    root.title("🖤 BLVCK TEA - Crypto Analysis Engine")
    ...
    blvck_tea = BLVCKTEAVisualSystem(frame, 600, 500)

    # Control panel
    control_frame = tk.Frame(root, bg="#0d0d0d")
    control_frame.pack(pady=10)

    states = ["idle", "connecting", "analyzing", "buy_signal", "sell_signal", "trading", "success", "error"]

    for i, state in enumerate(states):
        btn = tk.Button(
            control_frame,
            text=state.upper().replace("_", " "),
            command=lambda s=state: blvck_tea.set_state(s),
            bg="#1a1a1a", fg="#00ffaa",
    ...
    # Special effects
    special_frame = tk.Frame(root, bg="#0d0d0d")
    special_frame.pack(pady=5)

    pulse_btn = tk.Button(
        special_frame,
        text="🪙 CRYPTO PULSE",
        command=blvck_tea.crypto_pulse_effect,
        bg="#004433", fg="#00ffaa",
        font=("Orbitron", 10, "bold"),
        relief="flat", bd=1, padx=15, pady=8
    )
    pulse_btn.pack(side="left", padx=10)

    # Start animation
    blvck_tea.start_animation("idle")


    # Demo cycling
    def demo_cycle():
        current = states.index(blvck_tea.current_state) if blvck_tea.current_state in states else 0
        next_state = states[(current + 1) % len(states)]
        blvck_tea.set_state(next_state)
        root.after(4000, demo_cycle)
...
class EnhancedTradingBot:
    ...
    # เพิ่มส่วนนี้ใน setup_tabbed_interface() method ของ Sci9.py
    # หาบรรทัดที่สร้าง tabs และเพิ่มแท็บ strategies
    ...
    def setup_tabbed_interface(self):
        ...
        self.tab_main = self.notebook.add("📊 Dashboard")
        self.tab_config = self.notebook.add("🔧 Configuration")
        self.tab_strategies = self.notebook.add("🎯 Strategies")  # <-- เพิ่มบรรทัดนี้
        self.tab_trading = self.notebook.add("📈 Trading Control")
        self.tab_coin_analysis = self.notebook.add("🪙 Coin Analysis")
        self.tab_full_auto = self.notebook.add("🤖 Full Auto")
        self.tab_testing = self.notebook.add("⚙️ API Testing")
    ...
    # เพิ่ม method นี้ในคลาส EnhancedTradingBot ของ Sci9.py
    ...
    def save_strategy_settings(self):
        ...
        try:
            ...
            strategy_config = {
                'enabled_strategies': {k: v.get() for k, v in self.strategy_vars.items()},
            ...
            # Update main config
            self.config['strategy_settings'] = strategy_config
            ...
            self.log_message("💾 Strategy settings saved")

        except ValueError:
            messagebox.showerror("Error", "Invalid parameter values")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to save strategy settings: {str(e)}")

    # ================================
    # การแก้ไขที่ต้องทำใน Sci9.py
    # ================================

    # 1. ใน setup_tabbed_interface() method แก้ไขเป็น:
    def setup_tabbed_interface(self):
        """Setup tabbed interface"""
        self.notebook = ctk.CTkTabview(self.main_container)
        self.notebook.pack(fill="both", expand=True, padx=10, pady=10)
    ...
    # 2. ใน setup_enhanced_ui() method แก้ไขเป็น:
    def setup_enhanced_ui(self):
        """Setup the complete enhanced UI system"""
        ...
        self.setup_main_tab()
        self.setup_configuration_tab()
        self.setup_strategies_tab()  # <-- เพิ่มบรรทัดนี้
        self.setup_trading_tab()
        self.setup_coin_analysis_tab()
        self.setup_full_auto_tab()
        self.setup_testing_tab()
